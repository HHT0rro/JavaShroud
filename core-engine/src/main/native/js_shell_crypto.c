...
static void js_shell_aes256_expand(const unsigned char key[32], unsigned char expanded[240]) {
    int used = 32;
    unsigned char rcon = 0x01u;
    unsigned char temp[4];
    memcpy(expanded, key, 32u);
    while (used < 240) {
        memcpy(temp, expanded + used - 4, sizeof(temp));
        if ((used & 31) == 0) {
            unsigned char first = temp[0];
            temp[0] = (unsigned char)(JS_SHELL_AES_SBOX[temp[1]] ^ rcon);
            temp[1] = JS_SHELL_AES_SBOX[temp[2]];
            temp[2] = JS_SHELL_AES_SBOX[temp[3]];
            temp[3] = JS_SHELL_AES_SBOX[first];
            rcon = js_shell_aes_xtime(rcon);
        } else if ((used & 31) == 16) {
            for (int i = 0; i < 4; i++) temp[i] = JS_SHELL_AES_SBOX[temp[i]];
        }
        for (int i = 0; i < 4; i++, used++) expanded[used] = (unsigned char)(expanded[used - 32] ^ temp[i]);
    }
    js_shell_secure_wipe(temp, sizeof(temp));
}

static void js_shell_aes256_block(const unsigned char in[16], const unsigned char key[32], unsigned char out[16]) {
    unsigned char state[16], expanded[240], t;
    memcpy(state, in, sizeof(state));
    js_shell_aes256_expand(key, expanded);
    for (int i = 0; i < 16; i++) state[i] ^= expanded[i];
    ...
    js_shell_secure_wipe(state, sizeof(state));
    js_shell_secure_wipe(expanded, sizeof(expanded));
}

static void js_shell_gcm_inc32(unsigned char counter[16]) {
    for (int i = 15; i >= 12; i--) if (++counter[i] != 0u) break;
}

static void js_shell_gcm_multiply(unsigned char value[16], const unsigned char hash_subkey[16]) {
    unsigned char product[16] = {0};
    unsigned char factor[16];
    memcpy(factor, hash_subkey, sizeof(factor));
    for (unsigned int bit_index = 0; bit_index < 128u; bit_index++) {
        unsigned char mask = (unsigned char)(0u - ((value[bit_index / 8u] >> (7u - (bit_index & 7u))) & 1u));
    ...
    js_shell_secure_wipe(product, sizeof(product));
    js_shell_secure_wipe(factor, sizeof(factor));
}

static void js_shell_gcm_hash_block(unsigned char state[16], const unsigned char hash_subkey[16], const unsigned char block[16]) {
    for (unsigned int i = 0; i < 16u; i++) state[i] ^= block[i];
    js_shell_gcm_multiply(state, hash_subkey);
}

static void js_shell_gcm_hash_update(unsigned char state[16], const unsigned char hash_subkey[16], const unsigned char *data, size_t size) {
    unsigned char partial[16] = {0};
    while (size >= 16u) {
        js_shell_gcm_hash_block(state, hash_subkey, data);
        data += 16u;
        size -= 16u;
    }
    if (size != 0u) {
        memcpy(partial, data, size);
        js_shell_gcm_hash_block(state, hash_subkey, partial);
    }
    js_shell_secure_wipe(partial, sizeof(partial));
}

static void js_shell_gcm_store_be64(unsigned char out[8], uint64_t value) {
    for (unsigned int i = 0; i < 8u; i++) out[7u - i] = (unsigned char)(value >> (i * 8u));
}

int js_shell_aes256_gcm_decrypt(const unsigned char key[32], const unsigned char nonce[12], const unsigned char *aad, size_t aad_size, const unsigned char *ciphertext_and_tag, size_t ciphertext_and_tag_size, unsigned char *plain_out) {
    ...
    size_t ciphertext_size, offset;
    uint64_t block_count;
    int ok = 0;

    if (!ciphertext_and_tag || ciphertext_and_tag_size < 16u) return 0;
    ciphertext_size = ciphertext_and_tag_size - 16u;
    if (!key || !nonce || (!aad && aad_size != 0u) || (!plain_out && ciphertext_size != 0u)) goto cleanup;
    if (aad_size > (size_t)(UINT64_MAX / 8u) || ciphertext_size > (size_t)(UINT64_MAX / 8u)) goto cleanup;
    block_count = (uint64_t)(ciphertext_size / 16u) + (ciphertext_size % 16u != 0u ? 1u : 0u);
    if (block_count > (uint64_t)UINT32_MAX - 1u) goto cleanup;

    j0[15] = 1u;
    memcpy(j0, nonce, 12u);
    js_shell_aes256_block(hash_subkey, key, hash_subkey);
    js_shell_gcm_hash_update(ghash, hash_subkey, aad, aad_size);
    js_shell_gcm_hash_update(ghash, hash_subkey, ciphertext_and_tag, ciphertext_size);
    js_shell_gcm_store_be64(lengths, (uint64_t)aad_size * 8u);
    js_shell_gcm_store_be64(lengths + 8u, (uint64_t)ciphertext_size * 8u);
    js_shell_gcm_hash_block(ghash, hash_subkey, lengths);
    js_shell_aes256_block(j0, key, tag_mask);
    for (unsigned int i = 0; i < 16u; i++) calculated_tag[i] = (unsigned char)(ghash[i] ^ tag_mask[i]);
    ok = js_shell_consttime_equal(calculated_tag, ciphertext_and_tag + ciphertext_size, sizeof(calculated_tag));
    if (!ok) goto cleanup;

    memcpy(counter, j0, sizeof(counter));
    for (offset = 0u; offset < ciphertext_size; offset += 16u) {
        size_t take = ciphertext_size - offset < 16u ? ciphertext_size - offset : 16u;
        js_shell_gcm_inc32(counter);
        js_shell_aes256_block(counter, key, stream);
        for (size_t i = 0; i < take; i++) plain_out[offset + i] = (unsigned char)(ciphertext_and_tag[offset + i] ^ stream[i]);
    }

cleanup:
    if (!ok && plain_out && ciphertext_size != 0u) js_shell_secure_wipe(plain_out, ciphertext_size);
    js_shell_secure_wipe(hash_subkey, sizeof(hash_subkey));
    js_shell_secure_wipe(ghash, sizeof(ghash));
    js_shell_secure_wipe(j0, sizeof(j0));
    js_shell_secure_wipe(counter, sizeof(counter));
    js_shell_secure_wipe(stream, sizeof(stream));
    js_shell_secure_wipe(tag_mask, sizeof(tag_mask));
    js_shell_secure_wipe(calculated_tag, sizeof(calculated_tag));
    js_shell_secure_wipe(lengths, sizeof(lengths));
    return ok;
}
...
int js_shell_open_boot_kek_sidecar(
    ...
    static const unsigned char key_domain[] = "JavaShroud/BootKekSidecar/v1/key";
    unsigned char key_material[(sizeof(key_domain) - 1u) + 16u];
    unsigned char wrapping_key[32];
    unsigned int binding_acc = 0u;
    int ok = 0;
    memset(key_material, 0, sizeof(key_material));
    memset(wrapping_key, 0, sizeof(wrapping_key));
    if (!sidecar || !expected_binding || !kek_out || sidecar_size != 118u ||
        sidecar[0] != 'J' || sidecar[1] != 'S' || sidecar[2] != 'B' || sidecar[3] != 'K' ||
        sidecar[4] != 1u || sidecar[5] != 0u || sidecar[6] != 16u || sidecar[7] != 12u ||
        sidecar[8] != 48u || sidecar[9] != 0u ||
        !js_shell_consttime_equal(sidecar + 10u, expected_binding, 32u)) goto cleanup;
    for (size_t i = 0u; i < 32u; i++) binding_acc |= (unsigned int)expected_binding[i];
    if (binding_acc == 0u) goto cleanup;
    memcpy(key_material, key_domain, sizeof(key_domain) - 1u);
    memcpy(key_material + sizeof(key_domain) - 1u, sidecar + 42u, 16u);
    js_shell_hmac_sha256(expected_binding, 32u, key_material, sizeof(key_material), wrapping_key);
    ok = js_shell_aes256_gcm_decrypt(
        wrapping_key,
        sidecar + 58u,
        sidecar,
        70u,
...
cleanup:
    js_shell_secure_wipe(key_material, sizeof(key_material));
    js_shell_secure_wipe(wrapping_key, sizeof(wrapping_key));
    if (!ok && kek_out) js_shell_secure_wipe(kek_out, 32u);
    return ok;
}
