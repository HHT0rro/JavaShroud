...
pub fn reject_legacy_or_macos_name(name: &str) -> Result<(), PlatformError> {
    ...
    if lower == "meta-inf/jsrt"
        || lower.starts_with("meta-inf/jsrt/")
        || lower.contains("/jsrt/")
    {
        return Err(PlatformError::UnsupportedArtifact {
            reason: "retired META-INF/jsrt paths are not runtime inputs",
        });
    }
...
mod tests {
    ...
    fn retired_artifact_names_fail_closed() {
        ...
        assert!(reject_legacy_or_macos_name("qp-macho").is_err());
        ...
        assert!(reject_legacy_or_macos_name("META-INF/jsrt/runtime.dll").is_err());
