...
pub fn qp_magic() -> [u8; 4] {
    qp_crypto::derived_vm_magic().expect("name schedule")
}
pub const QP_AUTH_TAG_SIZE: usize = 32;
pub const QP_DIALECT_COMMITMENT_SIZE: usize = 32;
pub const QP_HEADER_SIZE: usize = 4 + 16 + QP_DIALECT_COMMITMENT_SIZE + 4 + 16 + 2 + 2 + 4 + 4;
pub const QP_MAX_BLOCKS: usize = 12;
pub const QP_MAX_FRAME_SIZE: usize = 32 * 1024 * 1024;
pub const QP_MAX_SECTION_SIZE: usize = 16 * 1024 * 1024;
pub const QP_MAX_CONSTANTS: usize = 65_535;
pub const QP_MAX_ROWS_PER_BLOCK: usize = 65_535;
pub const QP_MAX_INSTRUCTIONS: usize = 65_535;
pub const QP_MAX_OPERANDS_PER_INSTRUCTION: usize = 256;
pub const QP_MAX_EXCEPTIONS: usize = 4_096;
pub const QP_MAX_STATE_BINDING: usize = 4 * 1024;
pub const QP_MAX_METADATA_SIZE: usize = 16 * 1024;
...
const WIDE_REGISTER_COUNT_FLAG: u16 = 0x8000;
const REGISTER_COUNT_MASK: u16 = 0x7fff;
...
const QP_CLEAN_ENTRY_INTEGRITY_HEX: &[u8] = b"10429f6c";
...
impl Default for ParserLimits {
    fn default() -> Self {
        Self {
            max_frame_size: QP_MAX_FRAME_SIZE,
            max_section_size: QP_MAX_SECTION_SIZE,
            max_constants: QP_MAX_CONSTANTS,
            max_blocks: QP_MAX_BLOCKS,
            max_rows_per_block: QP_MAX_ROWS_PER_BLOCK,
            max_instructions: QP_MAX_INSTRUCTIONS,
            max_operands_per_instruction: QP_MAX_OPERANDS_PER_INSTRUCTION,
            max_exceptions: QP_MAX_EXCEPTIONS,
        }
    }
}
...
impl ParserLimits {
    fn validate(self) -> Result<Self, VmError> {
        if self.max_frame_size > QP_MAX_FRAME_SIZE
            || self.max_section_size > QP_MAX_SECTION_SIZE
            || self.max_constants > QP_MAX_CONSTANTS
            || self.max_blocks > QP_MAX_BLOCKS
            || self.max_rows_per_block > QP_MAX_ROWS_PER_BLOCK
            || self.max_instructions > QP_MAX_INSTRUCTIONS
...
impl<'a> VmParser<'a> {
    ...
    pub fn with_shared_dialect(
        ...
        if state_binding.len() > QP_MAX_STATE_BINDING {
            return Err(VmError::LengthTooLarge {
                field: "state binding",
                length: state_binding.len(),
                maximum: QP_MAX_STATE_BINDING,
            });
        }
        Ok(Self {
            material,
    ...
    pub fn parse(&self, frame: &[u8]) -> Result<VmProgram, VmError> {
        ...
        if frame.len() < QP_HEADER_SIZE + QP_AUTH_TAG_SIZE {
        ...
        let body_length = frame.len() - QP_AUTH_TAG_SIZE;
        let mut header = Cursor::new(&frame[..body_length]);
        let magic = header.read_fixed::<4>()?;
        if magic == qp_crypto::RETIRED_VM_MAGIC || magic != qp_magic() {
            return Err(VmError::InvalidMagic);
        }
        let nonce = header.read_fixed::<16>()?;
        let dialect_commitment = header.read_fixed::<QP_DIALECT_COMMITMENT_SIZE>()?;
        ...
        let session_material = frame_session_material(
    ...
    fn parse_authenticated(
        ...
        let expected_mac =
            frame_hmac_fields(session_material, seed, &[&nonce, &frame[..body_length]]);
    ...
    fn parse_sections(
        ...
        let (cp_key, cp_iv) =
            frame_aes_material(session_material, &nonce, seed, SECTION_CONSTANT_POOL, 0);
        ...
        let (exception_key, exception_iv) =
            frame_aes_material(session_material, &nonce, seed, SECTION_EXCEPTIONS, 0);
...
fn validate_state_binding(
    ...
    let valid = fields[0] == expected_entry_token.as_bytes()
        ...
        && fields[2] == QP_CLEAN_ENTRY_INTEGRITY_HEX
...
fn key_id_for(session_material: &[u8; 32], seed: u32, nonce: &[u8; 16]) -> u32 {
    let digest = frame_hmac(session_material, seed, &[&nonce[..]], b"qp-key-id");
...
fn parse_sealed_string(bytes: &[u8], build_key: &[u8; 32]) -> Result<VmString, VmError> {
    ...
    let expected = crypto::hmac_bytes(
        build_key,
        &[b"javashroud-qp-cp-string-tag-v3", &nonce, &ciphertext],
    );
    ...
    let (key, iv) = crypto::cp_string_material(
        build_key,
        b"javashroud-qp-cp-string-key-v3",
        b"javashroud-qp-cp-string-iv-v3",
        &nonce,
    );
    let mut ciphertext_wipe = ciphertext;
...
fn parse_block_rows(
    ...
    for row_index in 0..row_count {
        let row = match mode {
            0 => read_plain_row(&mut cursor, compact_regs)?,
            FLAG_REGISTER_ROW_ENVELOPE => {
                read_envelope_row(&mut cursor, seed, block_id, row_index, compact_regs)?
            }
            FLAG_MIXED_OPERAND_ENVELOPE => {
                read_mixed_row(&mut cursor, seed, block_id, row_index, compact_regs)?
            }
...
fn read_wide_plain_row(cursor: &mut Cursor<'_>) -> Result<RawRow, VmError> {
    Ok(RawRow {
...
fn read_plain_row(cursor: &mut Cursor<'_>, compact_regs: bool) -> Result<RawRow, VmError> {
    let opcode = cursor.read_u16_be()?;
    let flags = cursor.read_u16_be()?;
    let (dst, src_a, src_b) = if compact_regs {
        (
...
fn read_envelope_row(
    ...
    for (slot, field) in order.into_iter().enumerate() {
        ...
        } else if compact_regs && (2..=4).contains(&field) {
            u32::from(cursor.read_u8()?) ^ (mask & 0xff)
        } else {
...
fn read_mixed_row(
    ...
    match shape {
        0 => read_plain_row(cursor, compact_regs),
        1 => read_envelope_row(cursor, seed, block_id, row_index, compact_regs),
        2 => {
            let operand =
                cursor.read_u32_be()? ^ register_row_mix_word(seed, block_id, row_index, 0x42, 5);
            let (src_b, src_a, dst) = if compact_regs {
                (
                    u16::from(cursor.read_u8()?)
                        ^ (register_row_mix_word(seed, block_id, row_index, 0x43, 4) as u16 & 0xff),
                    u16::from(cursor.read_u8()?)
                        ^ (register_row_mix_word(seed, block_id, row_index, 0x44, 3) as u16 & 0xff),
                    u16::from(cursor.read_u8()?)
                        ^ (register_row_mix_word(seed, block_id, row_index, 0x45, 2) as u16 & 0xff),
                )
            } else {
                (
                    cursor.read_u16_be()?
...
fn parse_nested_rows(
    ...
    for row_index in 0..row_count {
        ...
        for (slot, field) in order.into_iter().enumerate() {
            ...
            let encoded_value = if field == 5 {
                cursor.read_u32_be()?
            } else {
                u32::from(cursor.read_u16_be()?)
            };
            ...
            let xor_mask = if field == 5 {
                value_mask
            } else {
                value_mask & 0xffff
            };
            fields[field] = (encoded_value ^ xor_mask) as i32;
        }
...
fn parse_metadata(
    ...
    if text.len() > QP_MAX_METADATA_SIZE {
        return Err(VmError::LengthTooLarge {
            field: "metadata",
            length: text.len(),
            maximum: QP_MAX_METADATA_SIZE,
        });
    }
    ...
    if fields[6].is_empty()
        || fields[6].len() > QP_MAX_STATE_BINDING
...
pub fn encode_iconst7_frame(material: &VmKeyMaterial) -> Result<Vec<u8>, VmError> {
    ...
    fn row(
        ...
        output[5] = 0;
        output[6] = 0;
        output[7..11].copy_from_slice(&(operand as u32).to_be_bytes());
        output
    }
    ...
    let tag = crypto::hmac_bytes(
        &build_key,
        &[
            b"javashroud-qp-cp-string-tag-v3",
            &string_nonce,
            &ciphertext,
        ],
    );
    ...
    let (entry_key, entry_iv) =
        frame_aes_material(&session, &nonce, seed, SECTION_CONSTANT_POOL_ENTRY, 0);
    let entry_cipher = aes128_ctr(&entry_key, &entry_iv, &entry_plain, QP_MAX_SECTION_SIZE)
    ...
    let (cp_key, cp_iv) = frame_aes_material(&session, &nonce, seed, SECTION_CONSTANT_POOL, 0);
    let cp_cipher = aes128_ctr(&cp_key, &cp_iv, &cp_container, QP_MAX_SECTION_SIZE)
    ...
    let (block_key, block_iv) = frame_aes_material(&session, &nonce, seed, SECTION_INSTRUCTIONS, 0);
    let block_cipher = aes128_ctr(&block_key, &block_iv, &block_plain, QP_MAX_SECTION_SIZE)
        .map_err(|_| VmError::InvalidHeader("block cipher"))?;

    let mut exception_plain = Vec::new();
    push_u16(&mut exception_plain, 0);
    let (exception_key, exception_iv) =
        frame_aes_material(&session, &nonce, seed, SECTION_EXCEPTIONS, 0);
    ...
    let wrapped_mask = frame_hmac(&session, 0, &[&nonce, &state_binding], b"qp-seed-wrap");
    let token = frame_hmac(&session, seed, &[&nonce, &state_binding], b"qp-seed-token");
    ...
    body.extend_from_slice(&qp_magic());
    ...
    let mac = frame_hmac_fields(&session, seed, &[&nonce, &body]);
...
pub fn iconst7_frame_state_binding(material: &VmKeyMaterial) -> Vec<u8> {
    ...
    binding.push(0);
    binding.extend_from_slice(QP_CLEAN_ENTRY_INTEGRITY_HEX);
    binding.push(0);
...
mod parser_tests {
    ...
    fn vm_dialect_differs_across_materials_and_round_trips() {
        ...
        assert_ne!(first.encode(ICONST), first.encode(IADD));
        assert_ne!(first.encode(IRETURN), first.encode(REG_META));
    }
