...
impl<H: ObjectOperations> VmExecutor<H> {
    ...
    fn execute_at_depth(
        ...
        // JVM local-variable indexes count category-2 values (long/double) as
        // two slots.  The JNI boundary supplies one VmValue per source
        // argument, so lay values out by slot width rather than by vector
        // index; otherwise a float/double following a long is read from the
        // wrong local and surfaces as a misleading VerifyError.
        let mut local_slot = 0usize;
        for value in arguments {
            if local_slot >= frame.locals.len() {
                return Err(VmError::LocalOutOfBounds);
            }
            frame.locals[local_slot] = value.clone();
            local_slot = local_slot
                .checked_add(if value.is_wide() { 2 } else { 1 })
