...
use qp_crypto::{Binding, BindingError};
use std::fmt;
use std::ops::Range;
...
impl<'a> Cursor<'a> {
    ...
    pub fn read_u16_be(&mut self) -> Result<u16, PageError> {
...
impl PageKind {
    ...
    pub const Vm: Self = Self::QpMethod;
    #[allow(non_upper_case_globals)]
...
pub type QpResourceKind = PageKind;
...
pub type QpPageLayout = PageLayout;
...
pub type QpResourceHeader = PageHeader;
...
pub type QpLeafIdentity = LeafIdentity;
...
pub type QpRouteMetadata = PageRoute;
...
fn validate_path(value: &str, field: &'static str) -> Result<(), PageError> {
    ...
    if lowered == "meta-inf/jsrt" || lowered.starts_with("meta-inf/jsrt/") {
        return Err(PageError::InvalidPath(field));
    }
    Ok(())
}
...
pub type QpPageDescriptor = PageDescriptor;

fn runtime_binding_digest_inner(binding: &[u8]) -> Result<Digest, PageError> {
    Ok(*qp_crypto::RuntimeBindingDigest::compute_bytes(binding)
...
pub fn authentication_tag(binding_digest: &[u8], payload: &[u8]) -> Result<Digest, PageError> {
    let digest =
        qp_crypto::Digest::from_slice(binding_digest).ok_or(PageError::InvalidLength {
            field: "runtime binding digest",
            expected: DIGEST_SIZE,
            actual: binding_digest.len(),
        })?;
    Ok(*qp_crypto::authentication_tag(&digest, payload)
        .map_err(crypto_error)?
        .as_bytes())
}
...
pub fn locate_current_frame(frame: &[u8]) -> Result<FrameView, PageError> {
    ...
    if magic.as_slice() == RETIRED_FRAME_MAGIC.as_slice()
        || magic.as_slice() != qp_crypto::derived_frame_magic().expect("name schedule").as_slice()
    {
...
fn crypto_error(error: qp_crypto::CryptoError) -> PageError {
    match error {
        qp_crypto::CryptoError::InvalidBinding(error) => binding_error(error),
        qp_crypto::CryptoError::PayloadTooLarge { size, max } => PageError::LengthTooLarge {
            field: "runtime payload",
            length: size,
            maximum: max,
        },
...
pub type QpPageEnvelopeForm = EnvelopeForm;
...
pub type QpPageEnvelope = PageEnvelope;
...
pub type QpLocator = Locator;
pub type QpLocatorBinding = LocatorBinding;
...
/// A page lease that borrows ciphertext owned by an immutable catalog bundle.
///
/// The owned [`PageLease`] remains available for callers that construct a
/// one-shot page from an independent `Vec<u8>`.  Runtime catalog pages already
/// live in an attached, authenticated bundle, so cloning every page into a new
/// vector only to authenticate it is unnecessary.  This borrowed variant keeps
/// the ciphertext reference read-only and owns only transient schedule and
/// plaintext buffers; the caller retains ownership of the catalog bytes.
pub struct BorrowedPageLease<'a> {
    state: LeaseState,
    encoded: &'a [u8],
    schedule: Option<PageCipherSchedule>,
    payload: Option<Vec<u8>>,
...
impl<'a> BorrowedPageLease<'a> {
    pub fn new(encoded: &'a [u8], schedule: PageCipherSchedule) -> Self {
        Self {
            state: LeaseState::Open,
            encoded,
            schedule: Some(schedule),
    ...
    pub fn open(encoded: &'a [u8], schedule: PageCipherSchedule) -> Self {
        Self::new(encoded, schedule)
    }
    ...
    pub fn is_wiped(&self) -> bool {
        self.schedule.is_none() && self.payload.is_none()
    }

    pub fn authenticate(&mut self, context: PageLeaseContext<'_>) -> Result<(), PageError> {
        if self.state != LeaseState::Open {
            return Err(PageError::InvalidState(
                "borrowed page lease can authenticate only from Open",
            ));
        }
        let schedule = self.schedule.take().ok_or(PageError::InvalidState(
            "borrowed page lease schedule is unavailable",
        ))?;
        let result = match PageLayout::from_variant(context.layout) {
            Ok(layout) => decode_page(
                self.encoded,
    ...
    pub fn payload(&self) -> Result<&[u8], PageError> {
        if self.state != LeaseState::Authenticated {
            return Err(PageError::InvalidState(
                "borrowed page lease payload requires authentication",
            ));
        }
        self.payload.as_deref().ok_or(PageError::InvalidState(
            "authenticated borrowed page lease has no payload",
        ))
    }
    ...
    pub fn consume(&mut self) -> Result<Vec<u8>, PageError> {
        if self.state != LeaseState::Authenticated {
            return Err(PageError::InvalidState(
                "borrowed page lease can consume only from Authenticated",
            ));
        }
        self.state = LeaseState::Consumed;
        self.payload.take().ok_or(PageError::InvalidState(
    ...
    pub fn release(&mut self) -> Result<(), PageError> {
        match self.state {
            ...
            LeaseState::Consumed => Err(PageError::InvalidState(
                "consumed borrowed page lease cannot be released",
            )),
            LeaseState::Released => Ok(()),
        }
    }
...
impl Drop for BorrowedPageLease<'_> {
    fn drop(&mut self) {
        self.wipe_buffers();
    }
}
...
mod tests {
    ...
    fn sha256_matches_current_domain_framing() {
        ...
        assert_eq!(
            &frame[..4],
            qp_crypto::derived_frame_magic().expect("magic").as_slice()
        );
        assert_eq!(
            frame,
    ...
    #[test]
    fn borrowed_lease_authenticates_without_mutating_catalog_ciphertext() {
        let (encoded, key, commitment, identity, fingerprint, layout) = page_fixture();
        let original = encoded.clone();
        let schedule = PageCipherSchedule::from_material(&key).expect("schedule");
        let mut lease = BorrowedPageLease::open(encoded.as_slice(), schedule);
        lease
            .authenticate(PageLeaseContext::new(
                &commitment,
                &identity,
            ...
            .expect("authenticate borrowed page");
        assert_eq!(lease.payload().expect("payload"), b"authenticated page payload");
        assert_eq!(encoded, original, "borrowed lease must not wipe catalog bytes");
        let consumed = lease.consume().expect("consume");
        assert_eq!(consumed, b"authenticated page payload");
        lease.wipe();
        assert_eq!(encoded, original, "lease cleanup must leave catalog bytes intact");
    }

    #[test]
    fn borrowed_lease_tamper_fails_closed_and_keeps_catalog_bytes() {
        let (mut encoded, key, commitment, identity, fingerprint, layout) = page_fixture();
        encoded[layout.body_offset()] ^= 1;
        ...
        lease.release().expect("release failed borrowed lease");
        assert_eq!(encoded, tampered, "borrowed lease must not mutate catalog bytes");
    }
}
