use qp_crypto::{authentication_tag, constant_time_eq, CryptoError, RuntimeBindingDigest};
use qp_crypto::{
    Binding, BindingError, Digest, DIGEST_SIZE, MAX_PAYLOAD_SIZE, PROTOCOL_MAGIC_SIZE, PROTOCOL_VERSION,
};
use std::fmt;

pub const AUTH_TAG_SIZE: usize = DIGEST_SIZE;
pub const MAX_FRAME_SIZE: usize =
    1 + 4 + DIGEST_SIZE + MAX_PAYLOAD_SIZE + AUTH_TAG_SIZE + PROTOCOL_MAGIC_SIZE;
...
impl RuntimeEnvelope {
    ...
    pub fn encode(binding: &Binding, payload: &[u8]) -> Result<Vec<u8>, ProtocolError> {
        ...
        writer.write_bytes(&qp_crypto::derived_frame_magic().expect("name schedule"))?;
...
fn locate(frame: &[u8]) -> Result<FrameView<'_>, ProtocolError> {
    ...
    if magic == qp_crypto::RETIRED_FRAME_MAGIC
        || magic != qp_crypto::derived_frame_magic().expect("name schedule")
    {
