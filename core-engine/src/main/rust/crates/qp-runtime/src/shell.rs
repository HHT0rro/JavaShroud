use qp_crypto::Digest;
use qp_shell::{LoadedArtifact, LoaderError, SupportedTarget};
use std::fmt;
...
impl ShellArtifact {
    pub fn validate(target: SupportedTarget, name: &str, bytes: &[u8]) -> Result<Self, ShellError> {
        let artifact = qp_shell::validate_artifact(target, name, bytes)?;
        Ok(Self {
            target,
            name: name.to_owned(),
            digest: qp_crypto::sha256(artifact.bytes()),
            artifact,
        })
    }
