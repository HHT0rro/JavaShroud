...
/**
 * Build-only NativeChunk source captured before artifact sealing has assigned
 * its final resource path.
 *
 * A native shell/handler producer owns the opaque handle and proof when this
 * candidate is created. The candidate deliberately has no runtime
 ...
 * sealing, then converted to [AkenPendingNativeChunk] after a final route is
 * reserved.
 */
internal class AkenNativeChunkCandidate private constructor(
    val pageIndex: Int,
    ...
    init {
        ...
        require(targetPageSize in AkenPageSizePolicy.DEFAULT.allowedSizes(AkenResourceKind.NativeChunk)) {
    ...
    internal fun identityPageKeyForBuild(): String {
        requireLive()
        return akenNativeChunkIdentityPageKey(logicalIdentityValue, pageIndex)
    }

    internal fun copyForBuild(): AkenNativeChunkCandidate {
        requireLive()
        return AkenNativeChunkCandidate(
            pageIndex = pageIndex,
     ...
     */
    internal fun toPendingPage(route: AkenNativeChunkPreSealRoute): AkenPendingNativeChunk {
        requireLive()
    ...
    companion object {
        ...
        fun create(
        ...
        ): AkenNativeChunkCandidate {
            targetPageSize?.let { requestedTargetSize ->
                require(requestedTargetSize in AkenPageSizePolicy.DEFAULT.allowedSizes(AkenResourceKind.NativeChunk)) {
                    "AKEN requested NativeChunk target size is unsupported"
                }
            }
            var generatedLayout: AkenPageLayout? = null
            val selectedVariant = try {
                layoutVariant ?: AkenPageLayout.create("native", random).also { generatedLayout = it }.variant
            } finally {
            ...
            return AkenNativeChunkCandidate(
                pageIndex = pageIndex,
                targetPageSize = targetPageSize
                    ?: AkenPageSizePolicy.DEFAULT.choose(AkenResourceKind.NativeChunk, random),
...
/** Build-only route correlation key for one logical encrypted native chunk. */
internal fun akenNativeChunkIdentityPageKey(
    logicalIdentity: ByteArray,
    pageIndex: Int,
): String {
    ...
    val digest = MessageDigest.getInstance("SHA-256").apply {
        ...
        update(AkenResourceKind.NativeChunk.id.toByte())
