...
internal data class QpEntryMetadata(
...
private const val QP_ARGUMENT_TAGS = "ZBCSIJFDL["
private const val QP_RETURN_TAGS = "VZBCSIJFDL["
private const val QP_CP_SEALED_STRING_TYPE = 0x06
private val QP_RETIRED_MAGIC = byteArrayOf(0x56, 0x42, 0x43, 0x35)
internal const val QP_DIALECT_COMMITMENT_BYTES = 32
private val QP_CP_STRING_KEY_DOMAIN = "javashroud-qp-cp-string-key-v3".toByteArray(Charsets.US_ASCII)
private val QP_CP_STRING_IV_DOMAIN = "javashroud-qp-cp-string-iv-v3".toByteArray(Charsets.US_ASCII)
private val QP_CP_STRING_TAG_DOMAIN = "javashroud-qp-cp-string-tag-v3".toByteArray(Charsets.US_ASCII)
...
internal fun QpBuildContext.deriveQpIdentity(owner: String, name: String, descriptor: String): String {
...
internal fun QpBuildContext.deriveQpOwnerIdentity(owner: String): String {
    val material = deriveVmBuildKey()
    return try {
        val mac = Mac.getInstance("HmacSHA256")
        mac.init(SecretKeySpec(material, "HmacSHA256"))
...
internal class QpSerializer(
    ...
    entryMetadata: QpEntryMetadata = QpEntryMetadata(),
    buildContext: QpBuildContext,
    structureEntropy: ByteArray = randomQpStructureEntropy(),
) : MethodVisitor(Opcodes.ASM9) {

    private val entryMetadata: QpEntryMetadata = entryMetadata.copy(
    ...
    private fun serializeWithActiveKey(): ByteArray {
        ...
        val nestedVmFlag = if (nestedVm) QP_FLAG_NESTED_VM else 0
        val registerRowEnvelopeFlag = if (registerRowEnvelope) QP_FLAG_REGISTER_ROW_ENVELOPE else 0
        val mixedOperandEnvelopeFlag = if (mixedOperandEnvelope) QP_FLAG_MIXED_OPERAND_ENVELOPE else 0
        val flags = QP_FLAG_ENCRYPTED_CP or QP_FLAG_BLOCK_ENCRYPTED or QP_FLAG_MAC or QP_FLAG_STATE_BOUND or
            QP_FLAG_AUTHENTICATED or QP_FLAG_PER_ENTRY_CP or QP_FLAG_PADDED or QP_FLAG_PER_BLOCK_ENCRYPT or
            QP_FLAG_REGISTER_EXECUTABLE or QP_FLAG_SUPER_OPERATORS or QP_FLAG_ZSTD_SECTIONS or
    ...
    private fun productionEvidenceFor(
        ...
        return ProductionMethodEvidence(
            ...
            methodEncodingSha256 = CandidateProductionBuildEvidence.sha256Hex(serializedQp),
        )
    }
    ...
    private fun lowerToLogicalProgram(metadataCpIndex: Int): VmLogicalProgram {
        ...
        groups.add(
            LogicalGroup(
                maskedOpcodeBase = opcodeDialect.encodeOpcode(QP_REG_META),
                primaryFlags = QP_REG_FLAG_EXECUTABLE,
                primaryDst = 0,
                primarySrcA = 0,
                primarySrcB = 0,
    ...
    private fun semanticSplitEligible(group: LogicalGroup): Boolean =
        serializationBuildContext.maxHardening &&
            group.maskedOpcodeBase != opcodeDialect.encodeOpcode(QP_REG_META) &&
            group.maskSlots == 1
    ...
    private fun semanticSplitRows(primary: VmRegisterInstruction, logicalIndex: Int, blockId: Int): List<VmRegisterInstruction> {
        ...
        return listOf(
            primary.copy(
                ...
                flags = primary.flags or QP_REG_FLAG_SEMANTIC_SPLIT,
                srcA = primary.srcA xor sourceShare,
                operand = primary.operand xor operandShare,
            ),
            VmRegisterInstruction(
                opcode = QP_REG_SEMANTIC_SHARE,
                flags = QP_REG_FLAG_SEMANTIC_SHARE,
                dst = opcodeShare,
                srcA = sourceShare,
                srcB = checksum,
    ...
    private fun partitionLogicalGroups(groups: List<LogicalGroup>): List<List<LogicalGroup>> {
        ...
        val ceiling = minOf(QP_MAX_LOGICAL_BLOCKS, total / 2).coerceAtLeast(1)
    ...
    private fun registerGroup(instruction: VmInstruction, instructionIndex: Int): LogicalGroup {
        ...
        val isSuperOperator = baseOpcode in QP_SUPER_BASE..QP_SUPER_INVOKE
        val flags = QP_REG_FLAG_EXECUTABLE or if (isSuperOperator) QP_REG_FLAG_SUPER else 0
    ...
    private fun foldedPredicateBranchGroup(instructionIndex: Int): LogicalGroup? {
        ...
        if (first.opcode !in QP_COMPARE_BUILDER_OPCODES || second.opcode !in QP_PREDICATE_BRANCH_OPCODES) return null
    ...
    private fun foldedSuperGroup(instructionIndex: Int): LogicalGroup? {
        ...
        if (first.opcode !in QP_CONST_OPCODES || second.opcode !in QP_FOLDED_FUSION_OPCODES) return null
    ...
    private fun polymorphicConstantPoolIndexMap(): IntArray {
        val n = constantPool.size
        val map = IntArray(n) { it }
        if (n <= 1) return map
        var state = effectiveBuildSeed.toLong() xor 0x51504D43L
        for (index in n - 1 downTo 1) {
            state = state * 6364136223846793005L + 1L
            val other = (((state ushr 33).toInt() and Int.MAX_VALUE) % (index + 1))
            val tmp = map[index]
            map[index] = map[other]
            map[other] = tmp
        }
    ...
    private fun remapLogicalProgramCpIndexes(
        ...
        var maskIndex = 0
        var metadataRemapped = false
        var lastDecoded = -1
        val encodedMeta = opcodeDialect.encodeOpcode(QP_REG_META)
        val remappedBlocks = program.blocks.map { block ->
            val remappedInsns = block.instructions.mapIndexed { rowIndex, insn ->
                ...
                if (consumesMaskSlot) {
                    // Folded super rows expand to two logical VM instructions in the
                    // native lowerer and therefore consume two opcode-mask slots.  Keep
                    // the CP remap cursor in lockstep with that logical expansion;
                    // continuation/share rows consume no slot of their own.
                    maskIndex += if ((insn.flags and QP_REG_FLAG_FOLDED) != 0) 2 else 1
                }
                lastDecoded = maskedOpcode
                val decodedOpcode = try {
                    opcodeDialect.decodeOpcode(maskedOpcode)
                } catch (_: IllegalArgumentException) {
                    -1
                }
                val logicalOperand = insn.operand xor (semanticShare?.operand ?: 0)
                val isMeta = decodedOpcode == QP_REG_META || maskedOpcode == encodedMeta
                val cpOpcode = when {
                    (insn.flags and QP_REG_FLAG_FOLDED) != 0 -> -1
                    (insn.flags and QP_REG_FLAG_SUPER) != 0 -> canonicalVmOpcode(insn.srcB)
                    else -> decodedOpcode
                }
                val newOperand = when {
                    isMeta && logicalOperand in cpIndexMap.indices -> {
    ...
    private fun writeConstantPoolEntry(out: java.io.ByteArrayOutputStream, entry: Any) {
        when (entry) {
            is SealedStringPoolEntry -> {
                out.write(QP_CP_SEALED_STRING_TYPE)
    ...
    private fun superOperatorOpcode(instruction: VmInstruction, instructionIndex: Int): Int? {
        val superOpcode = when (instruction.opcode) {
            VmOpcodes.VM_ICONST, VmOpcodes.VM_BIPUSH, VmOpcodes.VM_SIPUSH -> QP_SUPER_CONST
            VmOpcodes.VM_IADD, VmOpcodes.VM_ISUB, VmOpcodes.VM_IMUL,
            VmOpcodes.VM_IAND, VmOpcodes.VM_IOR, VmOpcodes.VM_IXOR,
            VmOpcodes.VM_ISHL, VmOpcodes.VM_ISHR, VmOpcodes.VM_IUSHR -> QP_SUPER_INT_ARITH
            VmOpcodes.VM_IFEQ, VmOpcodes.VM_IFNE, VmOpcodes.VM_IF_ICMPEQ, VmOpcodes.VM_IF_ICMPNE -> QP_SUPER_CMP_BRANCH
            VmOpcodes.VM_INVOKESTATIC, VmOpcodes.VM_INVOKEVIRTUAL, VmOpcodes.VM_INVOKESPECIAL, VmOpcodes.VM_INVOKEINTERFACE -> QP_SUPER_INVOKE
            else -> return null
        }
    ...
    private fun writeEnvelopeRegisterRow(
        ...
        order.forEachIndexed { slot, fieldIndex ->
            ...
            if (fieldIndex == 5) {
                writeU4(out, value)
            } else if (compactRegs && fieldIndex in 2..4) {
                writeU1(out, value and 0xFF)
            } else {
    ...
    private fun writeMixedOperandRow(
        ...
        when (shape) {
            0 -> {
                ...
                writeU2(out, instruction.flags)
                if (compactRegs) {
                    writeU1(out, instruction.dst)
                    writeU1(out, instruction.srcA)
                    writeU1(out, instruction.srcB)
                } else {
                    writeU2(out, instruction.dst)
                    writeU2(out, instruction.srcA)
                    writeU2(out, instruction.srcB)
            ...
            1 -> writeEnvelopeRegisterRow(out, blockId, rowIndex, instruction, compactRegs)
                ...
                if (compactRegs) {
                    ...
                    writeU1(out, maskedReg(0x45, 2, instruction.dst))
                } else {
                    writeU2(out, maskedReg(0x43, 4, instruction.srcB))
    ...
    /**
     * Non-nested rows use a compact one-byte register representation only when
     * every register-shaped field fits.  The logical register count alone is
     * not sufficient because srcA may carry a full-width encoded CFG target.
     * Bind the physical width choice in the block header so the native parser
     * cannot infer a different row geometry.
     */
    private fun compactNonNestedRegisterRows(block: VmLogicalBlock, registerCount: Int): Boolean =
        registerCount <= 255 && block.instructions.all { instruction ->
            instruction.dst in 0..255 && instruction.srcA in 0..255 && instruction.srcB in 0..255
        }
    ...
    private fun serializeNestedBlock(block: VmLogicalBlock, registerCount: Int): ByteArray {
        ...
        writeU4(out, profile)
        writeU4(out, dialect)
        writeU2(out, block.instructions.size * QP_NESTED_MICROS_PER_ROW)
        block.instructions.forEachIndexed { rowIndex, instruction ->
            val fields = intArrayOf(
                instruction.opcode and 0xFFFF,
                instruction.flags and 0xFFFF,
    ...
    private fun encodeInvokeDynamicConstant(
        ...
        if (normalized.bootstrapMethodHandle.owner == "java/lang/invoke/StringConcatFactory") {
            return encodeStringConcatConstant(normalized)
        }
    ...
    /**
     * Encode a StringConcatFactory call as a current-format, typed recipe that
     * the native host can hand back to the JVM's StringBuilder semantics.  The
     * recipe and static constants are hex-framed so arbitrary UTF-8 text and
     * bootstrap control characters cannot alter the VM constant grammar.
     */
    private fun encodeStringConcatConstant(indy: NormalizedInvokeDynamic): String {
        val bootstrap = indy.bootstrapMethodHandle
        val constants = if (bootstrap.name == "makeConcatWithConstants") {
            indy.bootstrapMethodArguments.toList().drop(1)
        } else {
            emptyList()
        }
        val recipe = if (bootstrap.name == "makeConcat") {
            "\u0001".repeat(Type.getArgumentTypes(indy.descriptor).size)
        } else {
            indy.bootstrapMethodArguments.firstOrNull() as? String
                ?: throw UnsupportedOperationException("StringConcatFactory recipe is missing")
        }
        val recipeHex = recipe.toByteArray(StandardCharsets.UTF_8)
            .joinToString("") { "%02x".format(it.toInt() and 0xFF) }
        val encodedConstants = constants.joinToString("|") { encodeStringConcatConstantValue(it) }
        return buildString {
            append("concat|")
            append(indy.descriptor)
            append('|')
            append(recipeHex)
            append('|')
            append(constants.size)
            if (encodedConstants.isNotEmpty()) {
                append('|')
                append(encodedConstants)
            }
        }
    }

    private fun encodeStringConcatConstantValue(value: Any): String {
        val (tag, text) = when (value) {
            ...
            is Type -> 'S' to value.descriptor
            else -> throw UnsupportedOperationException("unsupported StringConcatFactory constant ${value::class.java.name}")
        }
        val hex = text.toByteArray(StandardCharsets.UTF_8)
            .joinToString("") { "%02x".format(it.toInt() and 0xFF) }
        return "$tag:$hex"
    }
...
private const val QP_FLAG_MAC = 0x0004
private const val QP_FLAG_STATE_BOUND = 0x0008
private const val QP_FLAG_PER_BLOCK_ENCRYPT = 0x0010
private const val QP_FLAG_AUTHENTICATED = 0x0020
private const val QP_FLAG_PER_ENTRY_CP = 0x0040
private const val QP_FLAG_PADDED = 0x0080
private const val QP_FLAG_REGISTER_EXECUTABLE = 0x0100
private const val QP_FLAG_SUPER_OPERATORS = 0x0200
private const val QP_FLAG_ZSTD_SECTIONS = 0x0400
private const val QP_FLAG_BLOCK_DISPATCH = 0x0800
private const val QP_FLAG_NESTED_VM = 0x1000
private const val QP_FLAG_POLYMORPHIC_CP = 0x2000
private const val QP_FLAG_REGISTER_ROW_ENVELOPE = 0x4000
private const val QP_FLAG_MIXED_OPERAND_ENVELOPE = 0x8000
private const val QP_NESTED_MAGIC = 0x4E56
private const val QP_NESTED_FIELD_COUNT = 6
private const val QP_NESTED_MICROS_PER_ROW = QP_NESTED_FIELD_COUNT + 1
private const val QP_NESTED_FIELD_OPCODE_BASE = 0x7000
private const val QP_NESTED_COMMIT_OPCODE_BASE = 0x6000
private const val QP_NESTED_COMMIT_SLOT = 0x7F
private const val QP_SECTION_CONSTANT_POOL = 1
private const val QP_SECTION_CONSTANT_POOL_ENTRY = 9
private const val QP_SECTION_INSTRUCTIONS = 2
private const val QP_SECTION_EXCEPTIONS = 3
private const val QP_SECTION_PADDING = 8
private const val QP_STORED_ZSTD_FLAG = 0x80000000.toInt()
private const val QP_REG_FLAG_EXECUTABLE = 0x0001
private const val QP_REG_FLAG_SUPER = 0x0002
private const val QP_REG_FLAG_FOLDED = 0x0004
private const val QP_REG_FLAG_SEMANTIC_SPLIT = 0x0008
private const val QP_REG_FLAG_SEMANTIC_SHARE = 0x4000
private const val QP_REG_FLAG_CONTINUATION = 0x8000
private const val QP_WIDE_REGISTER_COUNT_FLAG = 0x8000
private const val QP_REGISTER_COUNT_MASK = 0x7FFF
private const val QP_MAX_LOGICAL_BLOCKS = 12
private const val QP_REG_META = 0xFD
private const val QP_REG_OPERAND_CONT = 0xFC
private const val QP_REG_SEMANTIC_SHARE = 0xF7
private const val QP_SUPER_BASE = 0xF8
private const val QP_SUPER_CONST = 0xF8
private const val QP_SUPER_INT_ARITH = 0xF9
private const val QP_SUPER_CMP_BRANCH = 0xFA
private const val QP_SUPER_INVOKE = 0xFB
private val QP_CONST_OPCODES = setOf(VmOpcodes.VM_ICONST, VmOpcodes.VM_BIPUSH, VmOpcodes.VM_SIPUSH)
private val QP_INT_ARITH_OPCODES = setOf(VmOpcodes.VM_IADD, VmOpcodes.VM_ISUB, VmOpcodes.VM_IMUL)
private val QP_COMPARE_BUILDER_OPCODES = setOf(
    VmOpcodes.VM_LCMP, VmOpcodes.VM_FCMPL, VmOpcodes.VM_FCMPG, VmOpcodes.VM_DCMPL, VmOpcodes.VM_DCMPG,
)
private val QP_PREDICATE_BRANCH_OPCODES = setOf(
    VmOpcodes.VM_IFEQ, VmOpcodes.VM_IFNE, VmOpcodes.VM_IFLT, VmOpcodes.VM_IFGE, VmOpcodes.VM_IFGT, VmOpcodes.VM_IFLE,
)
...
private val QP_FOLDED_FUSION_OPCODES = setOf(
    VmOpcodes.VM_IADD, VmOpcodes.VM_ISUB, VmOpcodes.VM_IMUL,
    VmOpcodes.VM_IAND, VmOpcodes.VM_IOR, VmOpcodes.VM_IXOR,
    VmOpcodes.VM_ISHL, VmOpcodes.VM_ISHR, VmOpcodes.VM_IUSHR,
)
private object QpCryptoScope {
...
private fun zstdCompressSection(bytes: ByteArray): ByteArray {
    return QpCompressionCodec.compress(bytes)
}

private data class QpStoredSection(val bytes: ByteArray, val compressed: Boolean) {
    val encodedLength: Int get() = bytes.size or if (compressed) QP_STORED_ZSTD_FLAG else 0
}

private fun compressCpEntrySection(bytes: ByteArray): QpStoredSection {
    val compressed = QpCompressionCodec.compress(bytes)
    return if (compressed.size < bytes.size) {
        QpStoredSection(compressed, compressed = true)
    } else {
        QpStoredSection(bytes, compressed = false)
    }
}
...
private const val QP_DERIVE_LABEL_VM_STRUCTURE = "javashroud-qp-vm-structure-v1"

private fun deriveQpStructureSeed(
...
private fun writeU1(out: java.io.ByteArrayOutputStream, value: Int) {
