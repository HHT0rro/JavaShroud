package io.github.hht0rro.javashroud.transforms.hiding
...
fun hideMembers(artifact: BytecodeArtifact, ruleMatches: List<RuleMatch>, params: Map<String, Any>): TransformResult {
    val matchedClassNames = eligibleClassNamesForAction(artifact.classArtifacts, ruleMatches, "member-hide")
    ...
    val updatedClassArtifacts = artifact.classArtifacts.map { classArtifact ->
        if (matchedClassNames.contains(classArtifact.summary.internalName)) {
            val hiddenBytes = hideClassMembers(classArtifact.bytes)
            if (!hiddenBytes.contentEquals(classArtifact.bytes)) {
                classCount++
                reanalyzedClassArtifact(classArtifact, hiddenBytes)
            } else {
                classArtifact
            }
        } else {
