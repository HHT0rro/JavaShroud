...
import io.github.hht0rro.javashroud.bytecode.applyExceptionDispatch
...
fun applyExceptionDispatchLogic(artifact: BytecodeArtifact, ruleMatches: List<RuleMatch>, params: Map<String, Any>): TransformResult {
    val matchedClassNames = excludeReflectionSurfaceSensitiveClasses(artifact, eligibleClassNamesForAction(artifact.classArtifacts, ruleMatches, "exception-dispatch"))
    ...
    val updatedClassArtifacts = artifact.classArtifacts.map { classArtifact ->
        if (matchedClassNames.contains(classArtifact.summary.internalName)) {
            val obfuscatedBytes = applyExceptionDispatch(classArtifact.bytes, seed)
