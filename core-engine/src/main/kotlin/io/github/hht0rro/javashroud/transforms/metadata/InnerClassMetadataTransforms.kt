...
import io.github.hht0rro.javashroud.bytecode.stripInnerClassMetadata
...
fun stripInnerClassMetadata(artifact: BytecodeArtifact, ruleMatches: List<RuleMatch>, params: Map<String, Any>): TransformResult {
 val matchedClassNames = eligibleClassNamesForAction(artifact.classArtifacts, ruleMatches, "inner-class-metadata")
 .filterNot { it.startsWith("dev/aide/jvmobf/") }
    ...
    val updatedClassArtifacts = artifact.classArtifacts.map { classArtifact ->
        if (matchedClassNames.contains(classArtifact.summary.internalName)) {
            val strippedBytes = stripInnerClassMetadata(classArtifact.bytes)
            if (!strippedBytes.contentEquals(classArtifact.bytes)) {
                classCount++
                reanalyzedClassArtifact(classArtifact, strippedBytes)
            } else {
                classArtifact
            }
        } else {
