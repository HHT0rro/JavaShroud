...
import io.github.hht0rro.javashroud.bytecode.applyStackMapTypePoison
...
fun applyStackMapPoison(artifact: BytecodeArtifact, ruleMatches: List<RuleMatch>, params: Map<String, Any>): TransformResult {
    val matchedClassNames = eligibleClassNamesForAction(artifact.classArtifacts, ruleMatches, "stackmap-type-poisoning")
    ...
    val updatedClassArtifacts = artifact.classArtifacts.map { classArtifact ->
        if (matchedClassNames.contains(classArtifact.summary.internalName)) {
            val obfuscatedBytes = applyStackMapTypePoison(classArtifact.bytes, seed)
