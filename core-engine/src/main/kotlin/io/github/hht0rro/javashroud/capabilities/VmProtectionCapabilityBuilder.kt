...
internal fun vmProtectionCapabilityBindings(): List<CapabilityBinding> = listOf(
    CapabilityBinding(
        ...
        compatibilityNotes = "VBC4-only 破坏性升级：要求 jni-microkernel-loader、平台 native 库和 Java 11+ 运行目标；旧资源、旧 profile 和低强度参数全部拒绝。Java 8/classfile 52 输入不是兼容目标。",
        ...
        params = listOf(
            ...
            fixedQpInvariant("vbc4StateBoundEncoding", "VBC4 固定行为：state-bound encoding 始终启用，不能关闭。"),
            fixedQpInvariant("vbc4HandlerMorphing", "VBC4 固定行为：handler morphing 始终启用，不能关闭。"),
            fixedQpInvariant("vbc4StrengthMax", "VBC4 固定行为：强度固定为 max，不提供低强度或兼容 profile。"),
            fixedQpInvariant("vbc4InterpreterDiversity", "VBC4 固定行为：native-only 路径始终启用构建期解释器多样化，不能关闭。"),
            fixedQpInvariant("vbc4HashedJniSymbols", "VBC4 固定行为：JNI VM 调用目标使用 per-build/per-method token 定位，热路径不传递明文符号。"),
            fixedQpInvariant("vbc4ExecutableRegisterIr", "VBC4 固定行为：native dispatcher 以 register IR 为主执行，stack opcode 只作为兼容输入。"),
            fixedQpInvariant("vbc4SuperOperators", "VBC4 固定行为：serializer 按方法随机结构 seed 折叠 super-operator 并纳入认证状态。"),
            fixedQpInvariant("vbc4IntegrityKeyBinding", "VBC4 固定行为：session integrity digest 参与 seed unwrap、block key 和 CP key 派生。"),
            fixedQpInvariant("vbc4EphemeralRootMaterial", "VBC4 固定行为：native 根材料只按需短生命周期派生，用后擦除。"),
        ),
    ),
)
