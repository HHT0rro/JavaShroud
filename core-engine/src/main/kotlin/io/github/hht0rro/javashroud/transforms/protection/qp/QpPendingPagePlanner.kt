...
internal object QpPendingPagePlanner {
    ...
    fun partitionAndWipe(
        ...
        try {
            partition(
                candidate = candidate,
                route = route,
                blockPlan = QpBlockClusterPlanner.plan(candidate, random),
    ...
    internal fun partitionAndWipe(
        ...
        try {
            partition(
                candidate = candidate,
                route = route,
                blockPlan = QpBlockClusterPlanner.plan(candidate, targetSizeForPage),
                callSiteProofForPage = callSiteProofForPage,
                random = random,
            )
        } finally {
    ...
    private fun partition(
        ...
        val pendingPages = ArrayList<QpPendingPage>(blockPlan.clusters.size)
        val partitions = ArrayList<QpPagePartition>(blockPlan.clusters.size)
        var completed = false
        try {
            ...
            blockPlan.clusters.forEachIndexed { ordinal, cluster ->
                ...
                try {
                    ...
                    val pending = QpPendingPage.create(
 ...
 */
internal data class QpPagePartition(
    val pageIndex: Int,
    val targetSize: Int,
 ...
 */
internal class QpPendingPageBatch private constructor(
    private var entryTokenValue: Long,
    private var resourcePathValue: String,
    private var serializedLengthValue: Int,
    partitions: List<QpPagePartition>,
    pages: List<QpPendingPage>,
) : AutoCloseable {
    private var partitionsValue: List<QpPagePartition> = partitions.toList()
    private var pagesValue: List<QpPendingPage> = pages.toList()
     ...
     */
    fun <T> consumePendingPagesForBuild(block: (List<QpPendingPage>) -> T): T {
