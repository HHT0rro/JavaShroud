...
import io.github.hht0rro.javashroud.bytecode.injectIllegalIdentifiers
...
fun applyIllegalIdentifiers(artifact: BytecodeArtifact, ruleMatches: List<RuleMatch>, params: Map<String, Any>): TransformResult {
    val matchedClassNames = excludeReflectionSurfaceSensitiveClasses(artifact, eligibleClassNamesForAction(artifact.classArtifacts, ruleMatches, "illegal-identifier-injection"))
    ...
    val updatedClassArtifacts = artifact.classArtifacts.map { classArtifact ->
        if (matchedClassNames.contains(classArtifact.summary.internalName)) {
            val obfuscatedBytes = injectIllegalIdentifiers(classArtifact.bytes, seed)
