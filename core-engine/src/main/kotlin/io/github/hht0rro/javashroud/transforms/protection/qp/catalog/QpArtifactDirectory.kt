...
class RuntimeBindingDigest(
    ...
    nameSeed: ByteArray = io.github.hht0rro.javashroud.transforms.protection.qp.currentNameSeed(),
) : AutoCloseable {
    ...
    companion object {
        ...
        private fun requireDigest(value: ByteArray, field: String) {
            if (value.size != QP_DIGEST_SIZE) {
                QpDirectoryException.fail(
                    QpDirectoryException.Code.INVALID_INPUT,
                    "$field must be $QP_DIGEST_SIZE bytes",
                )
            }
        }
        ...
        private fun computeDigest(
            ...
            val schedule = io.github.hht0rro.javashroud.transforms.protection.qp.qpNameSchedule(artifactCommitment, nameSeed)
            val domain = schedule.deriveDomain(
                io.github.hht0rro.javashroud.transforms.protection.qp.QpNameSchedule.ROLE_CRYPTO,
                io.github.hht0rro.javashroud.transforms.protection.qp.QpNameSchedule.LANE_RUNTIME_BINDING,
            )
            try {
                digest.update(domain)
            } finally {
                java.util.Arrays.fill(domain, 0)
                schedule.close()
            }
            updateRuntimeCanonical(
                digest,
...
class PageKey private constructor(
    private var resourceKindValue: QpResourceKind,
    ...
    init {
        if (rawValue.size != QP_PAGE_KEY_SIZE) {
            QpDirectoryException.fail(
                QpDirectoryException.Code.INVALID_INPUT,
                "page key must be exactly $QP_PAGE_KEY_SIZE bytes",
            )
        }
        if (pageIndexValue < 0) {
            QpDirectoryException.fail(
    ...
    companion object {
        ...
        fun fromHandle(handle: QpHandle): PageKey {
        ...
        fun fromBytes(bytes: ByteArray): PageKey {
            if (bytes.size != QP_PAGE_KEY_SIZE) {
                QpDirectoryException.fail(
                    QpDirectoryException.Code.INVALID_INPUT,
                    "page key must be exactly $QP_PAGE_KEY_SIZE bytes",
                )
            ...
            return try {
                val kind = QpResourceKind.fromId(copy[0].toInt() and 0xFF)
                    ?: QpDirectoryException.fail(
                        QpDirectoryException.Code.INVALID_INPUT,
...
class QpArtifactPage private constructor(
    ...
    init {
        ...
        if (bindingDigestValue.size != QP_DIGEST_SIZE) {
            QpDirectoryException.fail(
                QpDirectoryException.Code.INVALID_INPUT,
                "page binding digest must be $QP_DIGEST_SIZE bytes",
            )
        }
    }
    ...
    internal fun compareKey(other: QpArtifactPage): Int = keyValue.compareRaw(other.keyValue)
    ...
    internal fun copyForDirectory(): QpArtifactPage {
        requireLive("artifact page")
        return fromVerified(
            keyValue.copyOfKey(),
            relativePathValue,
    ...
    fun copy(): QpArtifactPage = copyForDirectory()
    ...
    override fun toString(): String =
        "QpArtifactPage(kind=$resourceKind, page=$pageIndex, path=$relativePathValue)"
...
class QpArtifactDirectory private constructor(
    ...
    private var nameSeedValue: ByteArray,
) : AutoCloseable {
    @Volatile
    private var wiped = false

    init {
        if (rootDigestValue.size != QP_DIGEST_SIZE) {
            QpDirectoryException.fail(
                QpDirectoryException.Code.INVALID_INPUT,
                "directory root digest must be $QP_DIGEST_SIZE bytes",
            )
        }
    }
    ...
    val entries: List<QpDirectoryEntry>
        get() {
            requireLive()
            return entriesValue.toList()
        }

    val pages: List<QpArtifactPage>
        get() = entries
    ...
    fun copyNameSeed(): ByteArray {
        requireLive()
        return nameSeedValue.copyOf()
    }

    fun encode(): ByteArray = QpDirectorySerializer.encode(this)

    fun lookup(key: PageKey): QpDirectoryEntry? {
        requireLive()
        var low = 0
    ...
    fun copy(): QpArtifactDirectory = QpArtifactDirectory.create(
        runtimeBindingDigest,
    ...
    internal fun entriesForWire(): List<QpArtifactPage> {
        requireLive()
        return entriesValue
    }
    ...
    fun wipe() {
        ...
        Arrays.fill(nameSeedValue, 0)
        nameSeedValue = ByteArray(0)
        wiped = true
    }
    ...
    companion object {
        ...
        fun create(
            ...
            val copies = ArrayList<QpArtifactPage>()
            ...
            return try {
                ...
                if (copies.size > QpDirectorySerializer.MAX_ENTRIES) {
                    QpDirectoryException.fail(
                ...
                val result = QpArtifactDirectory(runtime, copies, root, nameSeed.copyOf())
                transferred = true
                result
            } finally {
                if (!transferred) {
        ...
        fun fromPages(
            runtimeBindingDigest: RuntimeBindingDigest,
            pages: Iterable<QpArtifactPage>,
        ): QpArtifactDirectory = create(runtimeBindingDigest, pages)
        ...
        internal fun fromDecoded(
            runtimeBindingDigest: RuntimeBindingDigest,
            entries: MutableList<QpArtifactPage>,
            rootDigest: ByteArray,
            nameSeed: ByteArray,
        ): QpArtifactDirectory = QpArtifactDirectory(runtimeBindingDigest, entries, rootDigest, nameSeed.copyOf())
    }
}
...
internal fun validatePageFields(
    ...
    if (offset < 0 || storedLength <= 0 || storedLength > QpDirectorySerializer.MAX_STORED_LENGTH) {
        QpDirectoryException.fail(
            QpDirectoryException.Code.INVALID_INPUT,
            "page offset/length is outside the bounded non-negative range",
        )
    }
    if (descriptor.isEmpty() || descriptor.size > QpDirectorySerializer.MAX_DESCRIPTOR_SIZE) {
        QpDirectoryException.fail(
            QpDirectoryException.Code.FIELD_TOO_LARGE,
            "page descriptor length is outside its bound",
        )
    }
    if (envelope.isEmpty() || envelope.size > QpDirectorySerializer.MAX_ENVELOPE_SIZE) {
        QpDirectoryException.fail(
            QpDirectoryException.Code.FIELD_TOO_LARGE,
...
internal fun validateNormalizedRelativePath(value: String) {
    ...
    try {
        if (bytes.isEmpty() || bytes.size > QpDirectorySerializer.MAX_PATH_SIZE ||
            '\u0000' in value || '\\' in value || value.startsWith('/') || value.endsWith('/') ||
            value.split('/').any { it.isEmpty() || it == "." || it == ".." }
        ) {
            QpDirectoryException.fail(
                QpDirectoryException.Code.INVALID_PATH,
...
internal fun checkedLength(value: Int, field: String): Int {
    if (value < 0 || value > QpDirectorySerializer.MAX_DIRECTORY_SIZE) {
        QpDirectoryException.fail(
            QpDirectoryException.Code.LENGTH_OVERFLOW,
...
internal fun checkedLength(value: Long, field: String): Int {
    if (value < 0L || value > Int.MAX_VALUE.toLong() || value > QpDirectorySerializer.MAX_DIRECTORY_SIZE.toLong()) {
        QpDirectoryException.fail(
            QpDirectoryException.Code.LENGTH_OVERFLOW,
            "$field exceeds JVM bounds",
        )
...
private fun encodePageKey(
    ...
    val output = ByteArray(QP_PAGE_KEY_SIZE)
