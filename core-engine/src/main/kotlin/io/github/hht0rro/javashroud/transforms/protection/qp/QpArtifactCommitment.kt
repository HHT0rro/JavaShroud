 ...
 */
enum class QpCanonicalExclusionKind(
 ...
 */
data class QpCanonicalExclusionRange(
    val entryName: String,
    val offset: Int,
    val length: Int,
    val kind: QpCanonicalExclusionKind,
) {
    init {
        require(entryName.isNotEmpty() && '\u0000' !in entryName) {
 ...
 * [QpArtifactCommitment.reserve] and may therefore remain zero placeholders.
 ...
 */
class QpCanonicalReservation(
    val entryName: String,
    canonicalBytes: ByteArray,
    val rootShardRanges: List<QpRootShardRange> = emptyList(),
    val selfReferentialRanges: List<QpCanonicalExclusionRange> = emptyList(),
) {
 ...
 */
class QpArtifactCommitment private constructor(
    digest: ByteArray,
    rootShardRanges: List<QpRootShardRange>,
    selfReferentialRanges: List<QpCanonicalExclusionRange>,
) {
     ...
     */
    internal fun matchesWriterEquivalentEntriesForBuild(entries: Iterable<QpArtifactEntry>): Boolean {
        return try {
     ...
     */
    internal fun verifyRootShardsForBuild(entries: Iterable<QpArtifactEntry>): Boolean {
    ...
    override fun toString(): String = "QpArtifactCommitment(sha256)"

    private fun deriveRootShard(range: QpRootShardRange, rangeIndex: Int): ByteArray {
    ...
    companion object {
         ...
         *
         * Each [QpCanonicalReservation] supplies the final entry size plus all
         ...
         */
        fun reserve(
            entries: Iterable<QpCanonicalReservation>,
        ): QpArtifactCommitment {
            val reservations = entries.toList()
            val placeholders = ArrayList<QpArtifactEntry>(reservations.size)
            try {
                reservations.forEach { reservation ->
                    val bytes = reservation.copyCanonicalBytesForBuild()
                    try {
                        placeholders += QpArtifactEntry(reservation.entryName, bytes)
                    } finally {
                        Arrays.fill(bytes, 0)
                    }
                }
        ...
        fun compute(
            ...
            selfReferentialRanges: Iterable<QpCanonicalExclusionRange> = emptyList(),
        ): QpArtifactCommitment {
        ...
        private fun normalizeRootShardRanges(
            ...
            ranges: List<QpRootShardRange>,
        ): List<QpRootShardRange> {
            val sorted = ranges.sortedWith(
                compareBy<QpRootShardRange>({ it.entryName }, { it.offset }, { it.length }),
            )
        ...
        private fun normalizeSelfReferentialRanges(
            entriesByName: Map<String, QpArtifactEntry>,
            ranges: List<QpCanonicalExclusionRange>,
        ): List<QpCanonicalExclusionRange> {
            val sorted = ranges.sortedWith(
                compareBy<QpCanonicalExclusionRange>(
                    { it.entryName },
                    { it.offset },
                    { it.length },
                    { it.kind.framingId },
            ...
            var previous: QpCanonicalExclusionRange? = null
            sorted.forEach { range ->
                val entry = entriesByName[range.entryName]
 ...
 */
class QpLeafIdentity private constructor(
    ...
    fun matches(handle: QpHandle): Boolean {
    ...
    override fun toString(): String = "QpLeafIdentity(kind=${resourceKind.logicalName}, page=$pageIndex)"

    companion object {
        private const val MAX_LOGICAL_IDENTITY_SIZE = 64 * 1024

        fun fromHandle(handle: QpHandle, logicalIdentity: ByteArray): QpLeafIdentity {
            var encoded: ByteArray? = null
            var locator: ByteArray? = null
            var fingerprint: ByteArray? = null
            return try {
...
class QpProofMetadata private constructor(
...
class QpRouteMetadata private constructor(
    val leafIdentity: QpLeafIdentity,
    ...
    fun matches(handle: QpHandle): Boolean = leafIdentity.matches(handle)
    ...
    companion object {
        ...
        ): QpRouteMetadata = QpRouteMetadata(
            QpLeafIdentity.fromHandle(handle, logicalIdentity), resourcePath,
...
object QpSealingMetadata {
    fun leafIdentity(handle: QpHandle, logicalIdentity: ByteArray): QpLeafIdentity =
        QpLeafIdentity.fromHandle(handle, logicalIdentity)
    ...
    fun routingFor(
        ...
        layoutVariant: String,
        logicalBindingPath: String = resourcePath,
    ): QpRouteMetadata = QpRouteMetadata.fromHandle(
...
private class QpMetadataReader(private val bytes: ByteArray) {
