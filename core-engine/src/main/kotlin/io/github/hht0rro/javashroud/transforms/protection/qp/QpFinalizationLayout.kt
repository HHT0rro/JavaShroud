...
internal class QpPendingPage private constructor(
    ...
    internal val expectedStoredLength: Int
        get() {
            ...
            return try {
                layout = QpPageLayout.fromVariant(layoutVariant)
                layout.encodedLength(plaintextValue.size + QpPageCodec.GCM_TAG_SIZE)
            } finally {
                layout?.wipe()
            }
        }
    ...
    companion object {
        ...
        fun create(
            ...
            val selectedVariant = try {
                layoutVariant ?: QpPageLayout.create("qp", random).also { generatedLayout = it }.variant
            } finally {
                generatedLayout?.wipe()
            }
            val selectedTargetPageSize =
                targetPageSize ?: QpPageSizePolicy.DEFAULT.choose(QpResourceKind.QpMethod, random)
            return QpPendingPage(
                entryToken = entryToken,
                resourcePath = resourcePath,
                resourceOffset = resourceOffset,
                pageIndex = pageIndex,
        ...
        private fun validateLayout(variant: String) {
            var layout: QpPageLayout? = null
            try {
                layout = QpPageLayout.fromVariant(variant)
            } finally {
                layout?.wipe()
            }
        }
...
/**
 * One mutable final writer entry owned by [QpFinalizationLayout].
 *
 ...
 */
internal class QpFinalizationEntry private constructor(
    val name: String,
    bytes: ByteArray,
) : AutoCloseable {
    ...
    companion object {
        internal fun create(name: String, bytes: ByteArray): QpFinalizationEntry =
            QpFinalizationEntry(name, bytes)
    }
}
 ...
 */
internal class QpDispatchBinding private constructor(
    val entryToken: Long,
    val pageIndex: Int,
 ...
 */
internal class QpFinalizationLayout private constructor(
    private val commitmentValue: QpArtifactCommitment,
    private var finalEntriesValue: List<QpFinalizationEntry>,
    private var materializationValue: QpPageMaterialization?,
    private var emissionsValue: QpPageEmissionSet?,
    private var nativeInputsValue: List<QpLocatorCompileInput>,
) : AutoCloseable {
    @Volatile
    private var wiped: Boolean = false
    ...
    internal fun entriesForBuild(): List<QpFinalizationEntry> {
        requireLive()
     ...
     */
    internal fun <T> withNativeCompileInputsForBuild(block: (List<QpLocatorCompileInput>) -> T): T {
        requireLive()
    ...
    internal fun <T> withClassPageBindingsForBuild(
        block: (List<QpClassPageBinding>) -> T,
    ): T {
        requireLive()
        ...
        try {
            materialization.pagesForBuild()
                .asSequence()
                .filter { page ->
                    page.descriptorForBuild.resourceKind == QpResourceKind.EncryptedClassPage
                }
                .forEach { page -> bindings += QpClassPageBinding.fromMaterializedPage(page) }
            bindings.sortWith(
                compareBy<QpClassPageBinding> { binding -> binding.identityPageKeyForBuild() }
    ...
    private fun verifyAndCapturePage(
        ...
        nativeInput: QpLocatorCompileInput,
        entriesByName: Map<String, QpArtifactEntry>,
        ...
        try {
            ...
            val descriptor = QpPageDescriptor.decode(descriptorBytes)
            val route = descriptor.route
            if (
                descriptor.resourceKind != QpResourceKind.QpMethod ||
    ...
    companion object {
        ...
        fun materializeAndWipe(
            ...
            pendingPages: Iterable<QpPendingPage>,
            fixedEntries: Iterable<QpArtifactEntry>,
            pendingStringPages: Iterable<QpPendingTextPage> = emptyList(),
            ...
            val finalEntries = ArrayList<QpFinalizationEntry>()
            val rootRanges = rootShardRanges.toList()
            var materialization: QpPageMaterialization? = null
            var emissions: QpPageEmissionSet? = null
            var output: QpFinalizationLayout? = null
            ...
            fun reservePhysicalRange(
                ...
                selfReferential.getOrPut(resourcePath) { mutableListOf() } += QpCanonicalExclusionRange(
            ...
            try {
                ...
                for (page in pendingNativeChunks) nativeChunks += page
                require(
                    pages.isNotEmpty() ||
                ...
                classPages.forEach { page ->
                    ...
                    try {
                        val definitionKey = finalizationPageKey(
                            resourceKind = QpResourceKind.EncryptedClassPage,
                            logicalIdentity = identity,
                            pageIndex = page.pageIndex,
                        )
                ...
                nativeChunks.forEach { page ->
                    val identity = page.copyLogicalIdentityForBuild()
                    try {
                        val definitionKey = finalizationPageKey(
                            resourceKind = QpResourceKind.NativeChunk,
                            logicalIdentity = identity,
                            pageIndex = page.pageIndex,
                        )
                ...
                fixed.forEach { (name, bytes) ->
                    reservations += QpCanonicalReservation(
                        entryName = name,
                ...
                val reservedCommitment = QpArtifactCommitment.reserve(reservations)
                    ...
                    when (descriptor.resourceKind) {
                        ...
                        QpResourceKind.StringPage -> {
                            ...
                            try {
                                ...
                                compilerInputs += QpLocatorCompileInput.fromTypedPage(
                                    descriptor = descriptor,
                        ...
                        QpResourceKind.NativeChunk -> {
                            materializedNativeChunkCount += 1
                            val identity = descriptor.logicalIdentity
                            var proof: ByteArray? = null
                            try {
                                val definitionKey = finalizationPageKey(
                ...
                if (emittedQpPages.isNotEmpty()) {
                    ...
                    try {
                        emissions = QpPageEmissionSet.create(meshRoot, emittedQpPages)
                    } finally {
                ...
                finalBytes.forEach { (name, bytes) ->
                    try {
                        finalEntries += QpFinalizationEntry.create(name, bytes)
                    } finally {
                        Arrays.fill(bytes, 0)
                    }
                }
                val retainedMaterialization = checkNotNull(materialization)
                output = QpFinalizationLayout(
                ...
                if (!completed) {
                    ...
                    emissions?.wipe()
                    emittedQpPages.forEach { it.wipe() }
        ...
        fun reserve(
            ...
            try {
                ...
                return QpArtifactCommitment.reserve(reservations)
            } finally {
                fixed.values.forEach { Arrays.fill(it, 0) }
                pageBuffers.values.forEach { Arrays.fill(it, 0) }
            }
        ...
        private fun QpFinalizationLayout.verifyOwnedEntriesForBuild(): Boolean {
            val artifactEntries = ArrayList<QpArtifactEntry>()
            val sourceCopies = ArrayList<ByteArray>()
            return try {
                entriesForBuild().forEach { entry ->
                    val bytes = entry.copyBytesForBuild()
                    sourceCopies += bytes
                    artifactEntries += QpArtifactEntry(entry.name, bytes)
                }
        ...
        private fun validateNonOverlappingPageRanges(
            rangesByEntry: Map<String, List<QpCanonicalExclusionRange>>,
        ) {
