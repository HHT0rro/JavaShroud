 ...
 */
internal class QpClassPageDescriptor private constructor(
    val internalName: String,
    private var pagesValue: List<QpClassPageDescriptorPage>,
) : AutoCloseable {
    @Volatile
    ...
    fun <T> withPagesForBuild(block: (List<QpClassPageDescriptorPage>) -> T): T {
        requireLive()
    ...
    companion object {
        ...
        private const val MIN_PAGE_RECORD_BYTES: Int = Int.SIZE_BYTES + QpHandle.ENCODED_HANDLE_SIZE + Short.SIZE_BYTES + 1

        fun create(
            internalName: String,
            pages: Iterable<QpClassPageDescriptorPage>,
        ): QpClassPageDescriptor {
            val copies = pages.map { it.copyForBuild() }
            return try {
                QpClassPageDescriptor(internalName, copies)
        ...
        fun decodeForBuild(encoded: ByteArray): QpClassPageDescriptor {
            ...
            try {
                repeat(pageCount) { expectedIndex ->
                    ...
                    try {
                        pages += QpClassPageDescriptorPage.create(
                            pageIndex = pageIndex,
...
/** One owned handle/proof binding within an [QpClassPageDescriptor]. */
internal class QpClassPageDescriptorPage private constructor(
    val pageIndex: Int,
    ...
    fun copyForBuild(): QpClassPageDescriptorPage {
        requireLive()
        return QpClassPageDescriptorPage(pageIndex, encodedHandleValue, callSiteProofValue)
    }
    ...
    companion object {
        ...
        fun create(
            ...
            encodedHandle: ByteArray,
            callSiteProof: ByteArray,
        ): QpClassPageDescriptorPage = QpClassPageDescriptorPage(
            pageIndex = pageIndex,
...
internal fun isValidQpClassPageInternalName(value: String): Boolean {
