...
class NativeExtractDirectoryFailClosedTest {
    ...
    fun unwritable_or_non_directory_extract_paths_are_skipped() {
        val method = QpBridge::class.java.getDeclaredMethod("ensureNativeExtractDirectory", File::class.java)
    ...
    fun loader_fail_closes_when_no_extract_directory_is_usable() {
        val path = Path.of("src/main/java/io/github/hht0rro/javashroud/transforms/protection/qp/QpBridge.java")
        ...
        assertTrue(source.contains("qp:native-extract-unavailable"))
        assertTrue(source.contains("qp:native-resource-missing"))
        assertTrue(source.contains("qp:native-extract-digest-mismatch"))
        assertTrue(source.contains("qp:native-loaded-digest-mismatch"))
    }
}
