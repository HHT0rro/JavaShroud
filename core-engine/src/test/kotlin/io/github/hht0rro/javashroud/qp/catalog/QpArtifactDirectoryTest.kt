package io.github.hht0rro.javashroud.qp.catalog
...
class QpArtifactDirectoryTest {
    ...
    fun rust_directory_parser_uses_the_current_jsr1dir_contract() {
        ...
        assertTrue(rust.contains("RETIRED_DIRECTORY_MAGIC"), "Rust directory parser must reject retired magic")
        assertTrue(rust.contains("FORMAT_VERSION"))
        assertTrue(rust.contains("TEST_NAME_SEED"))
        assertTrue(rust.contains("x86_64-unknown-linux-gnu.2.17"))
        assertTrue(rust.contains("x86_64-apple-darwin"))
    }

    @Test
    fun retired_directory_magic_hex_fails_closed() {
        val hex = io.github.hht0rro.javashroud.transforms.protection.hardening.ProtectionFormat.RETIRED_DIRECTORY_MAGIC_HEX
        val retired = ByteArray(hex.length / 2) { index ->
            hex.substring(index * 2, index * 2 + 2).toInt(16).toByte()
        }
        val padded = retired + ByteArray(64)
        assertFailsWith<QpDirectoryException> {
            QpDirectoryParser.decode(padded)
        }
    }
    ...
    fun directory_encode_is_stable_across_locales() {
        ...
        val page = samplePage(runtime, 0, QpResourceKind.StringPage)
    ...
    fun canonical_order_is_independent_of_input_permutation() {
        ...
        val pages = samplePages(runtime)
        try {
            val baseline = QpDirectorySerializer.encode(runtime, pages)
            ...
            repeat(12) { round ->
                ...
                val candidate = QpDirectorySerializer.encode(
                    runtime,
                    order.map { pages[it] },
                )
    ...
    fun accessors_are_defensive_and_close_wipes_owned_material() {
        ...
        try {
            ...
            assertEquals(QpResourceKind.StringPage, entry.key.resourceKind)
    ...
    fun final_native_binding_rejects_wrong_size_and_all_zero_digests() {
        ...
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(ByteArray(31) { 1 }, abi, target, spec, profile)
        }
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(native, ByteArray(33) { 1 }, target, spec, profile)
        }
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(native, abi, target, ByteArray(0), profile)
        }
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(ByteArray(32), abi, target, spec, profile)
        }
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(native, ByteArray(32), target, spec, profile)
        }
        assertFailsWith<QpDirectoryException> {
            FinalRuntimeBinding(native, abi, target, ByteArray(32), profile)
        }
    }
    ...
    private fun assertReject(encoded: ByteArray, runtime: RuntimeBindingDigest) {
        assertFailsWith<QpDirectoryException> {
            QpDirectoryParser.decode(encoded, runtime)
        }
    }
    ...
    private fun samplePages(runtime: RuntimeBindingDigest): List<QpArtifactPage> = listOf(
        samplePage(runtime, 0, QpResourceKind.StringPage),
        samplePage(runtime, 1, QpResourceKind.StringPage),
        samplePage(runtime, 2, QpResourceKind.NativeChunk),
    )
    ...
    private fun layout(encoded: ByteArray, count: Int): Layout {
        var cursor = io.github.hht0rro.javashroud.transforms.protection.qp.catalog.QpDirectorySerializer.MAGIC_BYTES_SIZE + 4
        ...
        cursor += 32
        val blobCount = readU32(encoded, cursor)
        cursor += 4
        val pathLengths = ArrayList<Int>(blobCount)
        repeat(blobCount) {
            pathLengths += cursor
