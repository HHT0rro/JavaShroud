...
class QpClassPageDescriptorSourceRegistryTest {
    @Test
    fun class_local_descriptor_sources_are_scoped_contiguous_and_wiped() {
        ...
        val handleZero = ByteArray(QpHandle.ENCODED_HANDLE_SIZE) { index -> (index * 23 + 3).toByte() }
        val handleOne = ByteArray(QpHandle.ENCODED_HANDLE_SIZE) { index -> (index * 29 + 11).toByte() }
        ...
        val candidateZero = QpClassPageCandidate.create(
        ...
        val candidateOne = QpClassPageCandidate.create(
        ...
        var escapedSources: List<QpClassPageDescriptorSource> = emptyList()
        try {
            context.registerQpClassPageCandidatesForClass(
            ...
            assertTrue(context.hasQpClassPageDescriptorSources())
            ...
            val duplicateHandle = ByteArray(QpHandle.ENCODED_HANDLE_SIZE) { index -> (index * 41 + 19).toByte() }
            ...
            try {
                assertFalse(scopedCopy.hasQpClassPageDescriptorSources())
                assertFailsWith<IllegalStateException> {
                    scopedCopy.withQpClassPageDescriptorSourcesForBuild {
        ...
        assertFailsWith<IllegalStateException> {
            context.withQpClassPageDescriptorSourcesForBuild {
