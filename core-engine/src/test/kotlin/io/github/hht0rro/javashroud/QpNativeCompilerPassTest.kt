...
import io.github.hht0rro.javashroud.transforms.protection.QpNativeCompilerRequest
...
class QpNativeCompilerPassTest {
    ...
    fun r1_exposes_exactly_the_locked_runtime_targets() {
        ...
        assertFalse(QpNativeCompilerPass.RUST_TARGETS.keys.any { it.startsWith("macos") })
    }
    ...
    fun r1_cargo_command_is_locked_and_target_directory_is_explicit() {
        ...
        assertEquals(
            listOf("cargo", "zigbuild", "--locked", "--package", "qp-ffi", "--lib", "--release", "--target", "x86_64-pc-windows-gnu", "--target-dir", "build/windows"),
    ...
    fun windows_rust_flags_keep_the_current_native_exports() {
        val flags = QpNativeCompilerPass.rustFlagsForTest(
            RustToolchainProvisioner.WINDOWS_RUSTUP_TARGET,
        ...
        assertFalse(
            QpNativeCompilerPass.rustFlagsForTest(
                RustToolchainProvisioner.LINUX_RUNTIME_TARGET,
                "0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef",
            ).contains("/EXPORT:"),
        )
    ...
    fun specialization_changes_the_artifact_identity() {
        ...
        val first = QpNativeCompilerPass.nativeArtifactCacheKey(
        ...
        val second = QpNativeCompilerPass.nativeArtifactCacheKey(
            taskPlatform = "windows-x64",
            rustTarget = RustToolchainProvisioner.WINDOWS_RUSTUP_TARGET,
            outputName = "qp_ffi.dll",
            sourceDigest = source,
    ...
    fun cache_key_does_not_bind_master_or_resource_keys() {
        ...
        try {
            val left = QpNativeCompilerPass.nativeArtifactCacheKey(
                taskPlatform = "windows-x64",
                rustTarget = RustToolchainProvisioner.WINDOWS_RUSTUP_TARGET,
                outputName = "qp_ffi.dll",
                sourceDigest = source,
            ...
            val right = QpNativeCompilerPass.nativeArtifactCacheKey(
                taskPlatform = "windows-x64",
                rustTarget = RustToolchainProvisioner.WINDOWS_RUSTUP_TARGET,
                outputName = "qp_ffi.dll",
                sourceDigest = source,
    ...
    fun rust_orchestrator_source_does_not_carry_c_generation_or_secret_specialization() {
        val source = java.nio.file.Files.readString(
            java.nio.file.Path.of("src/main/kotlin/io/github/hht0rro/javashroud/transforms/protection/QpNativeCompilerPass.kt"),
        )
    ...
    fun recompiled_native_rejects_invalid_specialization_digest() {
        assertFailsWith<IllegalArgumentException> {
            QpNativeCompilerPass.RecompiledNative("windows-x64", "qp_ffi.dll", byteArrayOf(1), ByteArray(0))
        }
        assertFailsWith<IllegalArgumentException> {
            QpNativeCompilerPass.RecompiledNative("windows-x64", "qp_ffi.dll", byteArrayOf(1), ByteArray(32))
        }
        assertFailsWith<IllegalArgumentException> {
            QpNativeCompilerPass.RecompiledNative("windows-x64", "qp_ffi.dll", byteArrayOf(1), ByteArray(16) { 1 })
        }
    }
    ...
    fun recompiled_native_retains_specialization_digest() {
        ...
        val native = QpNativeCompilerPass.RecompiledNative(
            "windows-x64",
            "qp_ffi.dll",
            byteArrayOf(1),
            digest.copyOf(),
    ...
    private fun fixedContext(masterFill: Int = 1): QpBuildContext = QpBuildContext(
        masterKey = ByteArray(QP_MASTER_KEY_SIZE) { (it + masterFill).toByte() },
