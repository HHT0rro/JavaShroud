...
import io.github.hht0rro.javashroud.transforms.protection.qp.QpPageEmissionRequest
import io.github.hht0rro.javashroud.transforms.protection.qp.QpPageEmitter
...
class QpPageEmitterTest {
    ...
    fun emits_independent_pages_for_one_native_method_and_wipes_every_owner() {
        ...
        val methodIdentity = "fixture:qp-qp-emitter:method".encodeToByteArray()
        ...
        val output = QpPageEmitter.emitAndWipe(plan, listOf(request0, request1))
        try {
            ...
            assertEquals(
                listOf("META-INF/.qp/qp/method-0.bin", "META-INF/.qp/qp/method-1.bin"),
            ...
            pages.forEachIndexed { index, page ->
                ...
                try {
                    ...
                    assertEquals(QpResourceKind.QpMethod, descriptor.resourceKind)
            ...
            try {
                assertEquals(QpArtifactCommitment.DIGEST_SIZE, meshRoot.size)
            } finally {
    ...
    fun page_zero_for_distinct_methods_is_correlated_by_handle_not_global_page_index() {
        val commitment = ByteArray(QpArtifactCommitment.DIGEST_SIZE) { index -> (index * 23 + 5).toByte() }
        val firstIdentity = "fixture:qp-qp-emitter:first".encodeToByteArray()
        val secondIdentity = "fixture:qp-qp-emitter:second".encodeToByteArray()
        ...
        val firstRequest = requestFor(
            ...
            resourcePath = "META-INF/.qp/qp/first-method-page-0.bin",
        ...
        val secondRequest = requestFor(
            ...
            resourcePath = "META-INF/.qp/qp/second-method-page-0.bin",
            callSiteProof = secondProof,
        )
        Arrays.fill(firstIdentity, 0)
        Arrays.fill(secondIdentity, 0)

        val output = QpPageEmitter.emitAndWipe(plan, listOf(firstRequest, secondRequest))
        try {
            assertTrue(plan.isWiped())
            ...
            val firstEmission = checkNotNull(pagesByPath["META-INF/.qp/qp/first-method-page-0.bin"])
            val secondEmission = checkNotNull(pagesByPath["META-INF/.qp/qp/second-method-page-0.bin"])
