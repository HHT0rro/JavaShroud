...
class QpTextPageCandidateRouteTest {
    @Test
    fun candidate_defensively_owns_string_page_inputs_and_binds_only_a_matching_preseal_route() {
        ...
        var candidateCopy: QpTextPageCandidate? = null
        var route: QpTextRoute? = null
        ...
        try {
            ...
            route = QpTextRoute.create(
                identityPageKey = candidate.identityPageKeyForBuild(),
                logicalBindingPath = bindingPath,
                futureResourcePath = "META-INF/.qp/string/candidate-final.bin",
            )
            ...
            val mismatchedRoute = QpTextRoute.create(
                identityPageKey = candidate.identityPageKeyForBuild(),
                logicalBindingPath = "META-INF/.logical/string/other.bin",
                futureResourcePath = "META-INF/.qp/string/other-final.bin",
            )
            try {
    ...
    fun preseal_route_reservation_is_scoped_deterministic_and_collision_checked() {
        val later = QpTextRouteCandidateRef.create(
            identityPageKey = "page_z",
            logicalBindingPath = "META-INF/.logical/string/z.bin",
        )
        val earlier = QpTextRouteCandidateRef.create(
        ...
        try {
            reservation = QpTextRouteReservation.reserve(
                ...
                allocator = QpTextRouteAllocator { candidate, ordinal, reservedEntryPaths ->
                    ...
                    "META-INF/.qp/string/reserved-$ordinal.bin"
                },
            )
            ...
            reservation.withRoutesForBuild { routes ->
                ...
                assertEquals(
                    listOf(
                        "META-INF/.qp/string/reserved-0.bin",
                        "META-INF/.qp/string/reserved-1.bin",
                    ),
            ...
            val collision = QpTextRouteCandidateRef.create(
            ...
            try {
                assertFailsWith<IllegalArgumentException> {
                    QpTextRouteReservation.reserve(
                        ...
                        allocator = QpTextRouteAllocator { _, _, _ -> "META-INF/occupied.bin" },
                    )
                }
            } finally {
                collision.wipe()
            ...
            val duplicate = QpTextRouteCandidateRef.create(
                identityPageKey = "page_a",
            ...
            try {
                assertFailsWith<IllegalArgumentException> {
                    QpTextRouteReservation.reserve(
                        ...
                        allocator = QpTextRouteAllocator { _, ordinal, _ ->
                            "META-INF/.qp/string/duplicate-$ordinal.bin"
                        },
                    )
                }
            } finally {
