...
class QpResourceCodecTest {
    ...
    fun runtime_resource_codec_roundtrips_and_rejects_tampering() = withQpBuildContext(fixedRuntimeCodecContext()) {
        ...
        assertEquals(null, QpResourceCodec.decode(encoded), "tampered payload must fail MAC/hash validation")
    }

    @Test
    fun runtime_resource_codec_v8_ends_at_authenticated_mac_without_length_marker() =
        withQpBuildContext(fixedRuntimeCodecContext()) {
    ...
    @Test
    fun runtime_resource_codec_same_inputs_emit_different_authenticated_envelopes() = withQpBuildContext(fixedRuntimeCodecContext()) {
        val plain = "same-runtime-resource-plaintext".toByteArray(Charsets.UTF_8)
        val first = QpResourceCodec.encode(
            bytes = plain,
            kind = RuntimeResourceKind.VmBytecode,
            seed = 0x12345678,
            variantId = 7,
        ...
        val second = QpResourceCodec.encode(
            bytes = plain,
            kind = RuntimeResourceKind.VmBytecode,
            seed = 0x12345678,
            variantId = 7,
        ...
        assertContentEquals(plain, QpResourceCodec.decode(first), "first randomized envelope must round-trip")
        assertContentEquals(plain, QpResourceCodec.decode(second), "second randomized envelope must round-trip")
    }
    ...
    fun runtime_resource_codec_is_bound_to_build_local_resource_key() {
        ...
        val firstContext = QpBuildContext(
            masterKey = ByteArray(QP_MASTER_KEY_SIZE) { index -> (index * 11 + 7).toByte() },
            nativeSeed = 0x1357_2468L,
            jarLayoutDigest = ByteArray(QP_LAYOUT_DIGEST_SIZE) { index -> (index * 3 + 1).toByte() },
        )
        val secondContext = QpBuildContext(
            masterKey = ByteArray(QP_MASTER_KEY_SIZE) { index -> (index * 13 + 5).toByte() },
            nativeSeed = 0x2468_1357L,
            jarLayoutDigest = ByteArray(QP_LAYOUT_DIGEST_SIZE) { index -> (index * 7 + 9).toByte() },
        )

        val encoded = withQpBuildContext(firstContext) {
        ...
        assertContentEquals(plain, withQpBuildContext(firstContext) { QpResourceCodec.decode(encoded) }, "matching build context must decode its own runtime resources")
        assertEquals(null, withQpBuildContext(secondContext) { QpResourceCodec.decode(encoded) }, "a different build context must fail closed instead of decoding copied runtime resources")
    }

    @Test
    fun runtime_resource_codec_authenticates_header_body_and_tag() = withQpBuildContext(fixedRuntimeCodecContext()) {
        val encoded = QpResourceCodec.encode(
        ...
        for (offset in listOf(5, 21, 25, 121, encoded.size - 32)) {
            ...
            assertEquals(null, QpResourceCodec.decode(tampered), "tampering offset $offset must fail closed")
        }
    }

    @Test
    fun runtime_resource_codec_rejects_non_current_headers() = withQpBuildContext(fixedRuntimeCodecContext()) {
        val encoded = QpResourceCodec.encode(
        ...
        assertEquals(null, QpResourceCodec.decode(encoded), "Runtime resource codec must decode only current authenticated envelopes")
    }
    ...
    fun runtime_resource_codec_rejects_legacy_user_reachable_xor_stream_envelopes() {
        ...
        assertEquals(null, QpResourceCodec.decode(legacy), "Legacy stream/XOR runtime resource envelopes must fail closed")
    }

    @Test
    fun runtime_resource_codec_does_not_require_java_zstd_jni_for_runtime_resources() = withQpBuildContext(fixedRuntimeCodecContext()) {
        val highEntropy = ByteArray(512) { index -> (index * 131 + (index ushr 1) * 17).toByte() }
        val seed = 0x2468ACE0
        val requested = QpResourceCodec.encode(
            bytes = highEntropy,
        ...
        val forced = QpResourceCodec.encode(
            bytes = highEntropy,
            kind = RuntimeResourceKind.NativeLibrary,
            seed = seed,
            variantId = 11,
        ...
        assertContentEquals(highEntropy, QpResourceCodec.decode(requested), "runtime payload must round-trip")
        assertContentEquals(highEntropy, QpResourceCodec.decode(forced), "forced uncompressed payload must round-trip")
    }
    ...
    private fun fixedRuntimeCodecContext(): QpBuildContext = QpBuildContext(
        masterKey = ByteArray(QP_MASTER_KEY_SIZE) { index -> (index * 17 + 3).toByte() },
