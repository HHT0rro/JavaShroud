...
import io.github.hht0rro.javashroud.transforms.protection.qp.QpDispatchBinding
...
class QpFinalizationLayoutTest {
    @Test
    fun reserves_final_geometry_materializes_independent_pages_and_binds_native_compile_records() {
        ...
        val rootShard = QpRootShardRange(
            entryName = "io/example/Helper.class",
            offset = 17,
            length = QpArtifactCommitment.DIGEST_SIZE,
        )
        helperBytes.fill(0, rootShard.offset, rootShard.offset + rootShard.length)
        val fixedEntries = listOf(QpArtifactEntry("io/example/Helper.class", helperBytes))
        helperBytes.fill(0)
        ...
        val page0 = try {
            QpPendingPage.create(
                ...
                plaintext = plain0,
                resourcePath = "META-INF/.qp/qp/shared-page.bin",
                pageIndex = 0,
        ...
        try {
            ...
            var retainedDispatchBindings: List<QpDispatchBinding> = emptyList()
    ...
    fun finalization_rejects_overlapping_physical_page_ranges_before_plan_materialization() {
        val identity0 = "fixture:qp-finalization:overlap-0".encodeToByteArray()
        val identity1 = "fixture:qp-finalization:overlap-1".encodeToByteArray()
        val plain = "overlap page payload".encodeToByteArray()
        val proof = byteArrayOf(1, 2, 3, 4)
        val page0 = QpPendingPage.create(
            entryToken = 1L,
            logicalIdentity = identity0,
            plaintext = plain,
            resourcePath = "META-INF/.qp/qp/overlap.bin",
            pageIndex = 0,
            callSiteProof = proof,
            random = SecureRandom(),
        )
        val page1 = QpPendingPage.create(
            entryToken = 2L,
            logicalIdentity = identity1,
            plaintext = plain,
            resourcePath = "META-INF/.qp/qp/overlap.bin",
