...
class QpPendingTextPageTest {
    @Test
    fun pending_string_page_preserves_its_preassigned_handle_proof_frame_and_route_through_materialization() {
        val identity = "fixture:pending-string:identity".encodeToByteArray()
        val plaintext = "AKEN StringPage: UTF-8 ☃".encodeToByteArray()
        ...
        val resourcePath = "META-INF/.qp/string/pending-fixture.bin"
        val logicalBindingPath = "META-INF/.logical/string/pending-fixture.bin"
        val plan = newPlan()
        var pending: QpPendingTextPage? = null
        var materialization: QpPageMaterialization? = null
        try {
            pending = QpPendingTextPage.create(
            ...
            try {
                ...
                assertEquals(192, descriptor.targetPageSize)
            ...
            val other = try {
                QpPendingTextPage.create(
                    ...
                    resourcePath = "META-INF/.qp/string/pending-fixture-other.bin",
                    pageIndex = 2,
                    callSiteProof = otherProof,
                    encodedHandle = otherHandle,
                    targetPageSize = 192,
                    logicalBindingPath = "META-INF/.logical/string/pending-fixture-other.bin",
                )
            } finally {
    ...
    @Test
    fun malformed_pending_string_page_inputs_fail_closed_before_build_plan_registration() {
        ...
        val validHandle = ByteArray(QpHandle.ENCODED_HANDLE_SIZE) { index -> (index + 7).toByte() }
        try {
            assertFailsWith<IllegalArgumentException> {
                QpPendingTextPage.create(
                    logicalIdentity = validIdentity,
                    plaintext = validPlaintext,
                    resourcePath = "META-INF/.qp/string/invalid.bin",
                    pageIndex = 0,
                    callSiteProof = validProof,
                    encodedHandle = ByteArray(QpHandle.ENCODED_HANDLE_SIZE - 1),
                    targetPageSize = 128,
                    logicalBindingPath = "META-INF/.logical/string/invalid.bin",
                )
            }
            assertFailsWith<IllegalArgumentException> {
                QpPendingTextPage.create(
    ...
    fun wipe_invalidates_build_copies_and_prevents_repeat_materialization_input_creation() {
        val pending = QpPendingTextPage.create(
            logicalIdentity = "fixture:pending-string:wipe".encodeToByteArray(),
            plaintext = "erase owned buffers".encodeToByteArray(),
            resourcePath = "META-INF/.qp/string/wipe.bin",
            pageIndex = 0,
            callSiteProof = byteArrayOf(0x21, 0x22),
            encodedHandle = ByteArray(QpHandle.ENCODED_HANDLE_SIZE) { index -> (index * 3 + 1).toByte() },
            targetPageSize = 128,
            logicalBindingPath = "META-INF/.logical/string/wipe.bin",
        )
        val plan = newPlan()
        try {
            pending.wipe()
