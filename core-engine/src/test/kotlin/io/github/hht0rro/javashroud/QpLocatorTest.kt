...
class QpLocatorTest {
    ...
    fun locator_accepts_only_locked_native_platforms_and_binds_final_rows() {
        ...
        val linux = QpLocator.entry(
            platform = "linux-x64",
            resourcePath = "META-INF/final.so",
            fileSuffix = ".so",
            storedBytes = byteArrayOf(4, 5, 6),
        )
        val baselineDigest = QpLocator.finalNativeBindingDigest(listOf(windows, linux))
        val changedDigest = QpLocator.finalNativeBindingDigest(
            listOf(
                windows,
                QpLocator.entry(
                    platform = "linux-x64",
        ...
        assertFailsWith<IllegalArgumentException> {
            QpLocator.entry("macos-x64", "META-INF/final.dylib", ".dylib", byteArrayOf(1))
        }
        assertFailsWith<IllegalArgumentException> {
            QpLocator.entry("windows-x64", "META-INF/final.so", ".so", byteArrayOf(1))
        }
        assertFailsWith<IllegalArgumentException> {
            QpLocator.entry("linux-x64", "META-INF/final.dylib", ".so", byteArrayOf(1))
        }
        assertFailsWith<IllegalArgumentException> {
            QpLocator.entry("windows-x64", "META-INF/qp/final.dll", ".dll", byteArrayOf(1))
        }
        assertFailsWith<IllegalArgumentException> {
            QpLocator.entry("windows-x64", "META-INF/.qp/final.dll", ".dll", byteArrayOf(1))
        }
    ...
    fun catalogBindingInputs_matches_library_sha256_and_nonzero_abi_digest() {
        ...
        val entry = QpLocator.entry(
            platform = "windows-x64",
            resourcePath = "META-INF/final.dll",
            fileSuffix = ".dll",
            storedBytes = storedBytes,
        )
        QpLocator.catalogBindingInputs(listOf(entry)).use { inputs ->
            assertTrue(
            ...
            assertEquals("qp-rust-ffi-v1", inputs.payloadProfile)
    ...
    fun catalogBindingInputs_rejects_empty_entries() {
        assertFailsWith<IllegalArgumentException> {
            QpLocator.catalogBindingInputs(emptyList())
        }
    }
    ...
    fun catalogBindingInputs_wipe_zeros_owned_copies() {
        ...
        val inputs = QpLocator.catalogBindingInputs(listOf(entry))
    ...
    private fun sealFixture(
        ...
        QpBuildContext(
            masterKey = ByteArray(QP_MASTER_KEY_SIZE) { (it * 29 + 7).toByte() },
            nativeSeed = 0x5A17C0DEL,
            nameSeed = ByteArray(16) { it.toByte() },
            jarLayoutDigest = ByteArray(QP_LAYOUT_DIGEST_SIZE) { (it * 17 + 3).toByte() },
        ).also { context ->
        ...
        val artifact = testAttachedArtifact(
            ...
            jarEntries = listOf(
                ...
                JarEntryData("${qpResourceDir()}/windows-x64/qp_ffi.dll", windowsBytes),
                JarEntryData("${qpResourceDir()}/linux-x64/libqp_ffi.so", linuxBytes),
            ) + listOfNotNull(additionalNativeEntry),
        )
        requireQpBuildContext().publishNativeSpecializationDigests(
            mapOf(
                "windows-x64" to ByteArray(32) { (it + 1).toByte() },
                "linux-x64" to ByteArray(32) { (it + 2).toByte() },
            ),
    ...
    private companion object {
        ...
        val COMMITMENT_DOMAIN = "javashroud-qp-native-locator-commitment-v2".toByteArray(StandardCharsets.US_ASCII)
        val ROUTE_MASK_DOMAIN = "javashroud-qp-native-locator-route-mask-v2".toByteArray(StandardCharsets.US_ASCII)
        const val VERSION = 2
