...
class QpQpBuildContextBindingTest {
    ...
    fun native_build_plan_is_bound_to_exactly_one_artifact_commitment() {
        ...
        try {
            val first = context.initializeQpBuildPlan(commitment)
            val same = context.initializeQpBuildPlan(sameCommitment)
            assertSame(first, same)
            assertFailsWith<IllegalArgumentException> {
                context.initializeQpBuildPlan(differentCommitment)
            }
        } finally {
            context.wipe()
        ...
        assertTrue(context.qpBuildPlanOrNull() == null)
    }
}
