...
class QpMethodCandidateRegistryTest {
    @Test
    fun candidate_registry_is_build_only_defensive_and_wiped_at_each_scope_boundary() {
        ...
        val logicalMethod = QpMethodIdentity.create(
            ...
            descriptor = "(I)I",
            logicalVmResourcePath = "META-INF/qp/logical-method.bin",
        )
        val sourceCandidate = QpMethodCandidate.create(
        ...
        var escapedSnapshot: QpMethodCandidate? = null
        try {
            context.registerQpMethodCandidates(listOf(sourceCandidate))
            ...
            try {
                assertFailsWith<IllegalArgumentException> {
                    context.registerQpMethodCandidates(listOf(duplicate))
                }
            } finally {
                duplicate.wipe()
            }
            ...
            assertFailsWith<IllegalStateException> {
                context.withQpLocatorRecordsForBuild { error("unexpected native locator record callback") }
            }

            val scopedCopy = context.scopedCopy()
            try {
                assertFailsWith<IllegalStateException> {
                    scopedCopy.withQpMethodCandidatesForBuild { error("scoped copy must not contain plaintext candidates") }
                }
            } finally {
                scopedCopy.wipe()
            }
        ...
        assertFailsWith<IllegalStateException> {
            context.withQpMethodCandidatesForBuild { error("wiped context must not expose candidates") }
        }
    }
}
