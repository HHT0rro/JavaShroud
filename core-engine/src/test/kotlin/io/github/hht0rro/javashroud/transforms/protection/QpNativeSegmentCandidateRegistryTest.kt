...
class QpNativeSegmentCandidateRegistryTest {
    @Test
    fun native_chunk_candidate_registry_and_preseal_routes_are_defensive_scoped_and_wiped() {
        ...
        val sourceCandidate = QpNativeSegmentCandidate.create(
        ...
        var escapedCandidate: QpNativeSegmentCandidate? = null
        var escapedRef: QpNativeRouteCandidateRef? = null
        var escapedRoute: QpNativeRoute? = null
        var reservation: QpNativeRouteReservation? = null
        try {
            context.registerQpNativeSegmentCandidates(listOf(sourceCandidate))
            sourceCandidate.wipe()
            Arrays.fill(logicalIdentity, 0)
            Arrays.fill(plaintext, 0)
            Arrays.fill(proof, 0)
            Arrays.fill(handle, 0)
            assertTrue(context.hasQpNativeSegmentCandidates())
            ...
            context.withQpNativeRouteCandidateRefsForBuild { refs ->
            ...
            reservation = context.reserveQpNativeRoutes(
                ...
                allocator = QpNativeRouteAllocator { candidate, ordinal, reservedEntryPaths ->
                    ...
                    assertFalse("META-INF/.qp/native/context-$ordinal.bin" in reservedEntryPaths)
                    "META-INF/.qp/native/context-$ordinal.bin"
                },
            )
            assertEquals(reservation, context.requireQpNativeRouteReservation())
            reservation.withRoutesForBuild { routes ->
                ...
                assertEquals("META-INF/.qp/native/context-0.bin", route.futureResourcePath)
            }
            assertFailsWith<IllegalStateException> {
                checkNotNull(escapedRoute).futureResourcePath
            }

            val duplicate = QpNativeSegmentCandidate.create(
            ...
            try {
                assertFailsWith<IllegalArgumentException> {
                    context.registerQpNativeSegmentCandidates(listOf(duplicate))
                }
            } finally {
                duplicate.wipe()
            }
            ...
            try {
                assertFalse(scopedCopy.hasQpNativeSegmentCandidates())
                assertNull(scopedCopy.qpNativeRouteReservationOrNull())
                assertFailsWith<IllegalStateException> {
                    scopedCopy.withQpNativeSegmentCandidatesForBuild {
        ...
        assertFailsWith<IllegalStateException> {
            context.withQpNativeSegmentCandidatesForBuild {
