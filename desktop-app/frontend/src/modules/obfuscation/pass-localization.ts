...
const commonParamDescriptions: Readonly<Record<string, LocalizedText>> = {
  ...
  nativePackingLevel: text('native 加固档位：四档统一采用 AKEN v4 资源级 evaluator；off 不加外壳，standard 使用认证 overlay，max 使用 stub shell，max-hardening 使用更强的外壳与完整 js_kernel 认证 payload。档位只改变 native 外壳与载荷封装强度。', 'Native hardening level: all four values use the AKEN v4 resource-level evaluator; off has no outer shell, standard uses an authenticated overlay, max uses a stub shell, and max-hardening uses a stronger shell with a complete authenticated js_kernel payload. The level changes only Native shell and payload packing strength.'),
