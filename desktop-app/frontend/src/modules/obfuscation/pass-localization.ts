...
const commonParamDescriptions: Readonly<Record<string, LocalizedText>> = {
  ...
  nativePackingLevel: text('native 加固档位：四档统一采用当前资源级 evaluator；off 不加外壳，standard 使用认证 overlay，max 使用 stub shell，max-hardening 使用更强的外壳与完整 js_kernel 认证 payload。档位只改变 native 外壳与载荷封装强度。', 'Native hardening level: all four values use the current resource-level evaluator; off has no outer shell, standard uses an authenticated overlay, max uses a stub shell, and max-hardening uses a stronger shell with a complete authenticated js_kernel payload. The level changes only Native shell and payload packing strength.'),
  ...
  seed: text('种子：作为非秘密个性化输入；具体 pass 是否可复现以能力说明为准，Native runtime 输出始终包含每产物随机材料。', 'Seed: non-secret personalization input. Reproducibility depends on the pass; Native runtime output always includes per-artifact random material.'),
...
const passCopies: Readonly<Record<string, LocalizedPassCopy>> = {
  ...
  'method-virtualization': {
    ...
    description: text('把方法语义转换为 native VM 指令并由 Native runtime 执行，默认自动识别 VM 兼容关键方法以减少回退裸露面。', 'Converts method semantics into native VM instructions executed by the Native runtime, defaulting to critical VM-compatible method selection to reduce fallback exposure.'),
    params: {
      ...
      vbc4StateBoundEncoding: text('固定默认：state-bound encoding 始终开启，并作为隐藏的 fail-closed 不变量。', 'Fixed default: state-bound encoding is always on and remains a hidden fail-closed invariant.'),
      vbc4HandlerMorphing: text('固定默认：handler morphing 始终开启，并作为隐藏的 fail-closed 不变量。', 'Fixed default: handler morphing is always on and remains a hidden fail-closed invariant.'),
      vbc4StrengthMax: text('固定默认：强度固定为 max，不暴露低强度或兼容 profile。', 'Fixed default: strength is locked to max, with no low-strength or compatibility profile exposed.'),
      vbc4InterpreterDiversity: text('固定默认：native-only 路径始终启用解释器多样化；关闭会被引擎拒绝。', 'Fixed default: interpreter diversity is always enabled for the native-only path; disabling it is rejected by the engine.'),
      vbc4HashedJniSymbols: text('固定默认：JNI VM 目标使用构建和方法级 token 定位，避免在热路径传递明文符号。', 'Fixed default: JNI VM targets use build- and method-scoped tokens to avoid passing plaintext symbols on hot paths.'),
      vbc4ExecutableRegisterIr: text('固定默认：native dispatcher 以 register IR 执行，stack opcode 仅作为兼容输入。', 'Fixed default: the native dispatcher executes register IR, with stack opcodes retained only as compatible input.'),
      vbc4SuperOperators: text('固定默认：serializer 按方法随机结构 seed 折叠 super-operator，并纳入认证状态。', 'Fixed default: the serializer folds super-operators by per-method randomized structure seed and binds them into authenticated state.'),
      vbc4IntegrityKeyBinding: text('固定默认：session integrity digest 参与 seed unwrap、block key 和 CP key 派生。', 'Fixed default: the session integrity digest participates in seed unwrap, block-key, and CP-key derivation.'),
      vbc4EphemeralMasterKey: text('固定默认：native 主密钥只按需短生命周期派生，用后擦除。', 'Fixed default: the native master key is derived only for a short on-demand lifetime and wiped after use.'),
    },
  },
  'reference-proxy': {
