...
本轮重跑验证（2026-07-07，Windows 主机 + WSL Kali/Linux 路径）：`RealJniMicrokernelFixtureRegressionTest.jni_microkernel_loader_preserves_real_demo_and_complex_business_fixtures` 在 Windows 主机强制重跑 `BUILD SUCCESSFUL`，并刷新 `build/core-engine/reports/native-max/native-max-runtime-evidence.md`：报告记录 `runtime_status: passed`、baseline/obfuscated stdout 均为 `REAL_RESULT=1;LIFECYCLE=ok`、`tamper_probe_count: 5`、`tamper_fail_closed_count: 5`、max shell entry 数为 1、standard overlay marker 为 0。WSL Kali 使用同一测试入口和 Linux native target 路径重跑也已 `BUILD SUCCESSFUL in 14m 27s`，未生成新的 `hs_err_pid*.log`，覆盖 Linux manual-mapped ELF inner runtime 的真实 JAR 执行稳定性。随后组合回归 `NativeKernelShellPackerTest` XML 结果为 9 tests / 0 failures / 0 errors，覆盖 max payload header/parser/MAC、profile-bound header MAC、per-chunk tag mismatch 与 standard overlay 篡改边界；`NativeRecompilationTransformsTest.native_max_reverse_evidence_report_is_written_for_outer_stubs` 重新运行 `BUILD SUCCESSFUL in 2m 49s`，并刷新 `build/core-engine/reports/native-max/native-max-reverse-evidence.md`：报告中 `META-INF/js-native/js_kernel_{linux,windows,macos}` entry 数为 4、outer stub entry 数为 4、standard overlay entry 数为 0，四平台均记录 SHA-256、文件 magic、max marker、敏感明文命中、printable token 摘要和工具可用性；WSL `strings` 对四平台产物均返回 `exit_code: 0` 且 `forbidden_hits: none`，Linux ELF 的 `readelf -h -Ws` 与 `nm -D` 也返回 `exit_code: 0`、`forbidden_hits: none` 并确认可见 `JNI_OnLoad`。`SchemaCapabilitiesTest.jni_microkernel_loader_schema_exposes_native_recompilation_params` XML 结果为 1 test / 0 failures / 0 errors，验证 `nativePackingLevel=max` 默认值、`off|standard|max` 选项、standard overlay 与 max stub shell 描述、完整 `js_kernel` 保护声明；桌面端 `corepack yarn build` 通过，确认新增前端本地化文案没有破坏构建。其中 Linux artifact 测试覆盖 inner ELF header 不 raw 直嵌、payload marker 唯一、敏感 JNI/helper/native symbol ASCII 明文不暴露，macOS artifact 测试覆盖 `macos-x64`/`macos-arm64` outer stub 形态与显式 fail-closed reason。
